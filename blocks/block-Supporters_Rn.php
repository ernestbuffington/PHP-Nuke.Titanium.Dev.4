...
global $titanium_prefix, $titanium_db, $titanium_user, $admin, $admin_file;

if(!isset($admin_file)) { $admin_file = "admin"; }
$content = "<center>"._SP_SUPPORTEDBY."<br /><br />";
$j = 1;
while($j <= 5) {
  $sresult = $titanium_db->sql_query("SELECT `site_id` FROM `".$titanium_prefix."_nsnsp_sites` WHERE `site_status`='1'");
  $numrows = $titanium_db->sql_numrows($sresult);
  if($numrows==1) {
    list ($siteid) = $titanium_db->sql_fetchrow($sresult);
  } else if($numrows > 1) {
    $i = 1;
    while(list ($stid) = $titanium_db->sql_fetchrow($sresult)) {
      $abid[$i] = $stid;
      $i++;
    }
...
$result = $titanium_db->sql_query("SELECT `site_id`, `site_name`, `site_image` FROM `".$titanium_prefix."_nsnsp_sites` $sitelist");
while(list($site_id, $site_name, $site_image) = $titanium_db->sql_fetchrow($result)) {
