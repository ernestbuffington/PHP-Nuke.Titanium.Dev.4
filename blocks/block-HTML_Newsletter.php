...
global $pnt_db, $pnt_prefix, $msnl_gasModCfg, $msnl_sModuleNm;
...
if ( $msnl_gasModCfg['show_categories'] == 1 ) {  //Build SQL for displaying categories
	...
	$sql = "SELECT `nid`, nl.`cid`, `topic`, `sender`, `datesent`, `view`, `groups`, `hits`, "
				...
				.$pnt_prefix."_hnl_newsletters` as nl, `"
				.$pnt_prefix."_hnl_categories` nc WHERE nl.`cid` = nc.`cid` "
	...
	$sql = "SELECT `nid`, nl.`cid`, `topic`, `sender`, `datesent`, `view`, `groups`, `hits`, "
				."`ctitle`, `cblocklimit`  FROM `"
				.$pnt_prefix."_hnl_newsletters` as nl, `"
				.$pnt_prefix."_hnl_categories` nc WHERE nl.`cid` = nc.`cid` ORDER BY `datesent` DESC";
...
while ( $row = $pnt_db->sql_fetchrow( $msnl_result2 ) ) {
