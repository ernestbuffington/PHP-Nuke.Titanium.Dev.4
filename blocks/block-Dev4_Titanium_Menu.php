...
global $titanium_db, $admin, $user, $titanium_prefix, $titanium_user_prefix, $cookie, $def_module, $currentlang, $cookie, $cache;
...
$gestiongroupe = 1; // setting 0 forces Configurable Summary not to manage groups. (gain of 1 SQL query)
...
if ($is_user==1 && $detectPM==1) {
    $uid=intval($uid); // on sécurise l'appel à la BDD
     $newpms = $titanium_db->sql_fetchrow($titanium_db->sql_query("SELECT COUNT(*) FROM " . $prefix . "_bbprivmsgs WHERE privmsgs_to_userid='$uid' AND (privmsgs_type='5' OR privmsgs_type='1')")); //2 requetes SQL
}
...
if ($gestiongroupe==1) {
    ...
    $modulesaffiche= $titanium_db->sql_query($sql);
    while($tempo = $titanium_db->sql_fetchrow($modulesaffiche)) {
        $tempoA[] = $tempo;
    }
/*****[BEGIN]******************************************
 [ Base:    Caching System                     v3.0.0 ]
...
<?php
    ...
    $dynamictest=0;
    //Then, we load the nuke_summary table //
/*****[BEGIN]******************************************
 [ Base:    Caching System                     v3.0.0 ]
 ******************************************************/
				   ...
				   invisible, 
				       class, 
					    bold, 
						 new, 
					 listbox, 
					 dynamic FROM ".$titanium_prefix."_sommaire ORDER BY groupmenu ASC";
					 
    $result = $titanium_db->sql_query($sql);
    while ($row = $titanium_db->sql_fetchrow($result)) {  // we will display each category, then the corresponding modules //
        $row3[] = $row;
    }
/*****[BEGIN]******************************************
 [ Base:    Caching System                     v3.0.0 ]
 ...
 ******************************************************/
    ...
    $classpointeur=0;
    foreach($row3 as $row) {  // we will display each category, then the corresponding modules //
