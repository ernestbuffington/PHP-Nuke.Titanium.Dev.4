...
global $titanium_prefix, $titanium_db, $titanium_user, $admin, $admin_file, $titanium_cache;
...
$result = $titanium_db->sql_query("SELECT `site_id`, `site_name`, `site_url`, `site_image`, `site_date`, `site_description`, `site_hits` FROM `".$titanium_prefix."_nsnsp_sites` WHERE `site_status`>'0' ORDER BY `site_name` DESC");
if ((($image_atts = $titanium_cache->load('image_atts', 'nsnsp')) === false) || empty($image_atts)) {
    $image_atts = array();
    while(list($site_id, $site_name, $site_url, $site_image, $site_date, $site_description, $site_hits) = $titanium_db->sql_fetchrow($result)) {
    ...
    $titanium_cache->save('image_atts', 'nsnsp', $image_atts);
}
