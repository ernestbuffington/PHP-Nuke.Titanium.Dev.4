...
$totalselected = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$pnt_prefix."_nsnst_tracked_ips`"));
if(!isset($min)) {
  $min=0;
  ...
  $result = $pnt_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$pnt_prefix."_nsnst_tracked_ips` LIMIT $min, $perpage");
  while(list($xip_addr) = $pnt_db->sql_fetchrow($result)) {
    $xip_long = sprintf("%u", ip2long($xip_addr));
    list($xc2c) = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT `c2c` FROM `".$pnt_prefix."_nsnst_ip2country` WHERE `ip_lo`<='$xip_long' AND `ip_hi`>='$xip_long' LIMIT 0,1"));
    if(!$xc2c) { $xc2c = "00"; }
    $pnt_db->sql_query("UPDATE `".$pnt_prefix."_nsnst_tracked_ips` SET `c2c`='$xc2c' WHERE `ip_addr`='$xip_addr'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);
