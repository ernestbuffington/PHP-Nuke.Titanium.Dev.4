...
$totalselected = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$pnt_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname'"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $pnt_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$pnt_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname' ORDER BY `ip_long` LIMIT $min, $perpage");
  while(list($lipaddr) = $pnt_db->sql_fetchrow($result)){
    $newrow = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname' AND `ip_addr`='$lipaddr' ORDER BY `date` DESC LIMIT 1"));
