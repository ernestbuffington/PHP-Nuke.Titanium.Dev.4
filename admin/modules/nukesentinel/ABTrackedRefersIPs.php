...
$tid=intval($tid);
list($uname) = $titanium_db->sql_fetchrow($titanium_db->sql_query("SELECT `refered_from` FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `tid`='$tid' LIMIT 0,1"));
$totalselected = $titanium_db->sql_numrows($titanium_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname'"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $titanium_db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname' ORDER BY `ip_long` LIMIT $min, $perpage");
  while(list($lipaddr) = $titanium_db->sql_fetchrow($result)){
    $newrow = $titanium_db->sql_fetchrow($titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname' AND `ip_addr`='$lipaddr' ORDER BY `date` DESC LIMIT 1"));
