...
if(!isset($min)) {
  ...
  OpenTable();
  $testmessage = "";
  $result = $titanium_db->sql_query("SELECT `ip_lo`, `ip_hi` FROM `".$titanium_prefix."_nsnst_ip2country` ORDER BY ip_lo LIMIT $min, $perpage");
  while(list($xip_lo, $xip_hi) = $titanium_db->sql_fetchrow($result)) {
    $test1 = $titanium_db->sql_query("SELECT * FROM ".$titanium_prefix."_nsnst_ip2country WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_lo') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test2 = $titanium_db->sql_query("SELECT * FROM ".$titanium_prefix."_nsnst_ip2country WHERE (ip_lo<='$xip_hi' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test3 = $titanium_db->sql_query("SELECT * FROM ".$titanium_prefix."_nsnst_ip2country WHERE (ip_lo>='$xip_lo' AND ip_hi<='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
    $test4 = $titanium_db->sql_query("SELECT * FROM ".$titanium_prefix."_nsnst_ip2country WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
