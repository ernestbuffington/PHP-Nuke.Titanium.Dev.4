...
$totalselected = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsnst_excluded_ranges`"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsnst_excluded_ranges` ORDER BY $column $direction LIMIT $min,$perpage");
