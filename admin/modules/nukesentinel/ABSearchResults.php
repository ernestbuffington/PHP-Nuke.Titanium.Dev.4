...
if($totalselected > 0) {
  ...
  while($getIPs = $pnt_db->sql_fetchrow($result)) {
    list($getIPs['reason']) = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT `reason` FROM `".$pnt_prefix."_nsnst_blockers` WHERE `blocker`='".$getIPs['reason']."'"));
    ...
    list($bname) = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT `username` FROM `".$pnt_user_prefix."_users` WHERE `user_id`='".$getIPs['user_id']."'"));
...
if($totalselected > 0) {
  ...
  while(list($pnt_userid,$pnt_username,$ipaddr,$lastview,$hits,$tid, $c2c) = $pnt_db->sql_fetchrow($result)){
    ...
    echo "<td>";
    if($pnt_userid != 1) {
      echo "<a href='modules.php?name=Your_Account&amp;op=userinfo&amp;username=$pnt_username' target='_blank'><img src='images/nukesentinel/usericon.png' height='12' width='12' alt='$pnt_username' title='$pnt_username' border='0' /></a>";
    } else {
    ...
    echo "<td align='center'>&nbsp;<a href='".$admin_file.".php?op=ABPrintTrackedPages&amp;user_id=$pnt_userid&amp;ip_addr=$ipaddr' target='_blank'><img src='images/nukesentinel/print.png' height='16' width='16' alt='"._AB_PRINT."' title='"._AB_PRINT."' border='0' /></a>&nbsp;<a ";
    echo "href='".$admin_file.".php?op=ABTrackedPages&amp;user_id=$pnt_userid&amp;ip_addr=$ipaddr' target='_blank'><img src='images/nukesentinel/view.png' height='16' width='16' alt='"._AB_VIEW."' title='"._AB_VIEW."' border='0' /></a>&nbsp;<a ";
