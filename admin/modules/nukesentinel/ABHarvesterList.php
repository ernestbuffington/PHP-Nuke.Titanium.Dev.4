...
$totalselected = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsnst_harvesters`"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsnst_harvesters` ORDER BY `harvester` $direction LIMIT $min,$perpage");
