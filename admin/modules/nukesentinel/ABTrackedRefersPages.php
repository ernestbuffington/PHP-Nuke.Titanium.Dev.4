...
if(!isset($titanium_user_id)) $titanium_user_id='';
...
$totalselected = $titanium_db->sql_numrows($titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname'"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $titanium_db->sql_query("SELECT `ip_addr`, `page`, `date` FROM `".$titanium_prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname' ORDER BY $column $direction LIMIT $min, $perpage");
  while(list($lipaddr, $page, $date_time) = $titanium_db->sql_fetchrow($result)){
