...
OpenTable();
title(_CB_CONFIG2);
$cbinfo = $titanium_db->sql_fetchrow($titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsncb_config` WHERE `cgid`='2'"));
...
sort($cblockslist);
$result2 = $titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsncb_blocks` WHERE `cgid`='2' ORDER BY `cbid`");
