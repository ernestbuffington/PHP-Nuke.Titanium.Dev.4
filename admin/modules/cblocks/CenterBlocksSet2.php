...
OpenTable();
title(_CB_CONFIG2);
$cbinfo = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsncb_config` WHERE `cgid`='2'"));
...
sort($cblockslist);
$result2 = $pnt_db->sql_query("SELECT * FROM `".$pnt_prefix."_nsncb_blocks` WHERE `cgid`='2' ORDER BY `cbid`");
