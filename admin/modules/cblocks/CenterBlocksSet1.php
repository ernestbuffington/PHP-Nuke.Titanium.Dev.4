...
OpenTable();
title(_CB_CONFIG1);
$cbinfo = $titanium_db->sql_fetchrow($titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsncb_config` WHERE `cgid`='1'"));
...
sort($cblockslist);
$result2 = $titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsncb_blocks` WHERE `cgid`='1' ORDER BY `cbid`");
while($cbidinfo = $titanium_db->sql_fetchrow($result2)) {
