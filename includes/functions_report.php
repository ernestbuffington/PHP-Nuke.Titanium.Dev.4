...
function email_report($phpbb2_forum_id, $post_id, $topic_title, $comments)
{
    ...
    evo_phpmailer( $phpbb2_board_config['board_email'], $subject, $content, $headers );
...
function show_reports($status = REPORT_POST_NEW)
{
    ...
    if ( empty($phpbb2_forum_ids) )
    {
        return;
    }
    else
    {
        $where_sql2 = ' AND p.forum_id IN (' . implode(',', $phpbb2_forum_ids) . ')';
    }
 ...
 ******************************************************/
            'COMMENTS'            => $comments,
            'DATE'                => create_date($phpbb2_board_config['default_dateformat'], $row['report_time'], $phpbb2_board_config['board_timezone']),
...
function reports_count($status = REPORT_POST_NEW)
{
    ...
    {
        $where_sql = ' AND p.forum_id IN (' . implode(',', $phpbb2_forum_ids) . ')';
    }
...
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
            $last_action_date = create_date($phpbb2_board_config['default_dateformat'], $row['last_action_time'], $phpbb2_board_config['board_timezone']);
