...
function get_post_count($recync_user_id)
{
	...
	$post_count = $pnt_db->sql_fetchrow($result);
...
function topic_max_post_recync($recync_topic_id)
{
    ...
    $sql = "SELECT topic_moved_id
    	...
    	$result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Не могу получить данные о теме!", "", __LINE__, __FILE__, $sql);
    	$topic = $pnt_db->sql_fetchrow($result);
	...
	$sql = "SELECT MAX(post_id) as last_post
       ...
       $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not obtain topic MAX post_id!", "", __LINE__, __FILE__, $sql);
	   $new_id = $pnt_db->sql_fetchrow($result);

	$sql = 'UPDATE ' . TOPICS_TABLE . '
		SET topic_last_post_id = ' . $new_id['last_post']. '
		WHERE topic_id = ' . $recync_topic_id;
        $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not update topic_last_post_id!", "", __LINE__, __FILE__, $sql);
}
...
function topic_min_post_recync($recync_topic_id)
{
    ...
    $sql = "SELECT topic_moved_id
	  ...
	  $sql = "SELECT MIN(post_id) as first_post
		FROM " . POSTS_TABLE . "
		WHERE topic_id = $topic_id";
      $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not obtain topic MIN post_id", "", __LINE__, __FILE__, $sql);
	  $new_id = $pnt_db->sql_fetchrow($result);
	  ...
	  $sql = "UPDATE " . TOPICS_TABLE . "
		...
		WHERE topic_id = $recync_topic_id";
      $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not update topic_first_post_id", "", __LINE__, __FILE__, $sql);
}
...
function topic_replyes_count_decrease($post_id)
{
	global $pnt_db;
      ...
      $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not get topic_replyes data for this topic by its post", "", __LINE__, __FILE__, $sql);
      if (($row = $pnt_db->sql_numrows($result)) > '0' )
      {
	       	$replyes_num = $pnt_db->sql_fetchrowset($result);
         	foreach($replyes_num as $val)
         	{
                ...
                if ($new_post_count != '0')
                {
	                ...
	                WHERE topic_id =' . $topic_id;
	                $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not update replyes count for this topic by its post", "", __LINE__, __FILE__, $sql);
				}
            }
      }
}
...
function forum_postscount_decrease_by_post_id($post_id)
{
	global $pnt_db;
    	...
    	$result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not get forum data for this post!", "", __LINE__, __FILE__, $sql);
     	if (($row = $pnt_db->sql_numrows($result)) > '0' )
     	{
	      	$forums = $pnt_db->sql_fetchrowset($result);
        	foreach($forums as $val)
        	{
				...
				if ($new_post_count != '0')
                {
	                ...
	                $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not update posts count for this forum!", "", __LINE__, __FILE__, $sql);
				}
	        }
     }
}
...
function set_forum_last_post($recynced_forum_id)
{
	...
	$sql = "SELECT MAX(post_id) as last_post
      ...
      $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not obtain forum MAX post_id", "", __LINE__, __FILE__, $sql);
	  $lastpost = $pnt_db->sql_fetchrow($result);
      //
	...
	$sql = "UPDATE " . FORUMS_TABLE . "
	  ...
	  WHERE forum_id = $recynced_forum_id";
      $result = $pnt_db->sql_query($sql) or message_die(GENERAL_ERROR, "Could not update forum_last_post_id", "", __LINE__, __FILE__, $sql);
}
...
function forum_topics_count_minus_one($phpbb2_forum_id)
{
	global $pnt_db;
      ...
      WHERE forum_id =' . $phpbb2_forum_id;
	    ...
	    $phpbb2_topics = $pnt_db->sql_fetchrow($result);
