...
/*****[END]********************************************
 [ Mod:    Simple Subforums                    v1.0.1 ]
 ******************************************************/
                if ( $phpbb2_is_auth_ary[$row['forum_id']]['auth_read'] && $ignore_forum != $row['forum_id'] )
                {
 ...
 ******************************************************/
			...
			if ( $phpbb2_is_auth_ary[$row['forum_id']]['auth_read'] )
			{
				...
				{
					$row = $list[$j];
					if( $row['forum_parent'] == $phpbb2_parent_id && $phpbb2_is_auth_ary[$row['forum_id']]['auth_read'] && $ignore_forum != $row['forum_id'] )
					{
						if( $parent_hidden )
						{
        ...
        switch($type)
        {
                ...
                case 'all topics':
                  ...
                  case 'forum':
                        ...
                        {
                                $phpbb2_last_post = ( $row['last_post'] ) ? $row['last_post'] : 0;
                                $phpbb2_total_posts = ($row['total']) ? $row['total'] : 0;
                        }
                        else
                        {
                                $phpbb2_last_post = 0;
                        ...
                        $total_phpbb2_topics = ( $row = $pnt_db->sql_fetchrow($result) ) ? ( ( $row['total'] ) ? $row['total'] : 0 ) : 0;

                        $sql = "UPDATE " . FORUMS_TABLE . "
                                SET forum_last_post_id = '$phpbb2_last_post', forum_posts = '$phpbb2_total_posts', forum_topics = '$total_phpbb2_topics'
                                WHERE forum_id = '$id'";
