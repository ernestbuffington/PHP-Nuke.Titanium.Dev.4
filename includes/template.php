...
class Template {
	...
	var $titanium_cache_search = array();
	var $titanium_cache_replace = array();
 ...
 ******************************************************/
	...
	function set_filename($handle, $filename, $xs_include = false, $quiet = false)
	{
		...
		{
			$titanium_cache_time = @filemtime($this->files_cache[$handle]);
			if(@filemtime($this->files[$handle]) > $titanium_cache_time || $phpbb2_board_config['xs_template_time'] > $titanium_cache_time)
			{	
	...
	*/
	 ...
	 */
	function compile2($code, $handle, $titanium_cache_file)
	{
		$code = $this->compile_code('', $code, XS_USE_ISSET);
		if($titanium_cache_file && !empty($this->use_cache) && !empty($this->auto_compile))
		{
			$res = $this->write_cache($titanium_cache_file, $code);
			if($handle && $res)
			{
				$this->files_cache[$handle] = $titanium_cache_file;
			}
		}
		$code = '?'.'>'.$code.'<'."?php\n";
		return $code;
