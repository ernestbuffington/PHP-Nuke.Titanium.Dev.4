...
if(!isset($pollID)) {
    ...
    $resu = $pnt_db->sql_query("SELECT pollID, pollTitle, voters FROM ".$pnt_prefix."_poll_desc WHERE artid='0' ORDER BY timeStamp DESC LIMIT 1,6");
    while (list($plid, $pltitle, $plvoters) = $pnt_db->sql_fetchrow($resu)) {
        if ($pollID == $plid) {
...
function pollMain($pollID) {
    global $boxTitle, $boxContent, $pollcomm, $pnt_user, $pnt_prefix, $pnt_db, $pnt_module;
...
function pollLatest() {
    ...
    $result = $pnt_db->sql_query("SELECT pollID FROM ".$pnt_prefix."_poll_desc WHERE artid='0' $querylang ORDER BY pollID DESC LIMIT 1");
    $pollID = $pnt_db->sql_fetchrow($result);
    $pnt_db->sql_freeresult($result);
    return($pollID[0]);
}

function pollCollector($pollID, $voteID, $forwarder) {
    global $pnt_prefix, $pnt_db, $pnt_config, $identify;
    $ip = $identify->get_ip();
    $number_of_days = intval($pnt_config['poll_days']);
    $past = time()-86400*$number_of_days;
    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_poll_check WHERE time < $past");
    $result = $pnt_db->sql_query("SELECT ip FROM ".$pnt_prefix."_poll_check WHERE ip='$ip' AND pollID='$pollID'");
    list($ips) = $pnt_db->sql_fetchrow($result);
    $pnt_db->sql_freeresult($result);
    if ($ip != $ips) {
        $ctime = time();
        $pnt_db->sql_query("INSERT INTO ".$pnt_prefix."_poll_check (ip, time, pollID) VALUES ('$ip', '$ctime', '$pollID')");
        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_poll_data SET optionCount=optionCount+1 WHERE pollID='$pollID' AND voteID='$voteID'");
        if (!empty($voteID)) {
            $pnt_db->sql_query("UPDATE ".$pnt_prefix."_poll_desc SET voters=voters+1 WHERE pollID='$pollID'");
        }
    }
...
function pollList() {
    global $pnt_user, $pnt_prefix, $multilingual, $currentlang, $admin, $pnt_db, $pnt_module, $admin_file;
    ...
    $querylang = "";
    if ($multilingual) { $querylang = "AND planguage='$currentlang' OR planguage=''"; }
    $result = $pnt_db->sql_query("SELECT pollID, pollTitle, voters FROM ".$pnt_prefix."_poll_desc WHERE artid='0' $querylang ORDER BY timeStamp DESC");
    while(list($plID, $plTitle, $voters) = $pnt_db->sql_fetchrow($result)) {
...
function pollResults($pollID) {
    global $resultTableBgColor, $resultBarFile, $Default_Theme, $pnt_user, $pnt_prefix, $pnt_db, $admin, $pnt_module, $admin_file;

    if(!isset($pollID)) $pollID = 1;
    $result = $pnt_db->sql_query("SELECT pollID, pollTitle, artid FROM ".$pnt_prefix."_poll_desc WHERE pollID='$pollID'");
    $holdtitle = $pnt_db->sql_fetchrow($result);
    $pnt_db->sql_freeresult($result);
    echo "<strong>$holdtitle[1]</strong><br /><br />";

    $result = $pnt_db->sql_query("SELECT SUM(optionCount) FROM ".$pnt_prefix."_poll_data WHERE pollID='$pollID'");
    list($sum) = $pnt_db->sql_fetchrow($result);
    $pnt_db->sql_freeresult($result);
    echo "<table border=\"0\">";

    /* cycle through all options */
    $result = $pnt_db->sql_query("SELECT optionText, optionCount FROM ".$pnt_prefix."_poll_data WHERE pollID='$pollID' AND optionText!='' ORDER BY voteID");
    while(list($optionText, $optionCount) = $pnt_db->sql_fetchrow($result)) {
