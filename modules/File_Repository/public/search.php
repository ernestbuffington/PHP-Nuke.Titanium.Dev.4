...
{
	global $titanium_db, $titanium_lang_new, $titanium_module_name, $settings, $themes;
	OpenTable();
	...
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(false,'center',_sh(),2).'>'._suh($titanium_lang_new[$titanium_module_name]['SEARCH_DATABASE']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%','right',_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['SEARCH_QUERY']).'</td>'."\n";
	echo '    <td'._tdcss('50%',false,_sc()).'>'._input('text','query','250px','').'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(1).'>'."\n";
	echo '    <td'._tdcss('50%','right',_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['CATEGORY']).'</td>'."\n";
	echo '    <td'._tdcss('50%',false,_sc()).'>'._category_parents_and_children('cid','',true).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '  <tr'._bgColor(2).'>'."\n";
	echo '    <td'._tdcss(false,'center',_sf(),2).'>'._submit($titanium_lang_new[$titanium_module_name]['SEARCH']).'</td>'."\n";
	echo '  </tr>'."\n";
	echo '</table>'."\n";
	...
	if($query <> '')
	{
		...
		echo '<table style="width: 100%;" border="0" cellpadding="4" cellspacing="1" class="forumline">'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,'center',_sh(),(($settings['download_view'] == 0) ? '6' : $themes[get_theme()]['per_row'])).'>'._suh(sprintf($titanium_lang_new[$titanium_module_name]['SEARCH_RESULTS'],$query)).'</td>'."\n";
		echo '  </tr>'."\n";
		...
		if($numrows > 0)
		{
			if($settings['download_view'] == 0)
			{
				...
				{
					$v 			= (($q['version']) ? sprintf($titanium_lang_new[$titanium_module_name]['V'],$q['version']) : '');
					...
					{
						echo '    <td'._tdcss('5%','center',_sc()).'><a'._image_viewer($screen[$items]['filename'],'item-'.$q['did']).' href="'._FILE_REPOSITORY_SCREENS.$screen[$items]['filename'].'" title="'.$screen[$items]['title'].'"><img style="border: 1px solid white; height: 28px; width: 30px;" src="modules/'.$titanium_module_name.'/files/screenshots/'.$screen[$items]['filename'].'" border="0" /></a></td>';
						echo '    <td'._tdcss('45%',false,_sc()).'>';
						echo '      <a'._ls().' href="modules.php?name='.$titanium_module_name.'&amp;action=view&amp;did='.$q['did'].'">'._colorization($iteminfo['title'],$iteminfo['color']).$v.'</a>'._item_is_new($iteminfo['isnew'],$iteminfo['updated'])._mostpopular($iteminfo['hits']);
						echo '    </td>';
					} else {
			...
			else 
			{
				...
				while($q = $titanium_db->sql_fetchrow($result))
				{
					...
					echo '                  '._sut($titanium_lang_new[$titanium_module_name]['DOWNLOAD']).' : <a'._ls().' href="modules.php?name='.$titanium_module_name.'&amp;action=view&amp;did='.$q['did'].'">'._colorization($iteminfo['title'],$iteminfo['color']).'</a>'._item_is_new($iteminfo['isnew'],$iteminfo['updated'])._mostpopular($iteminfo['hits']);
					echo '                </td>'."\n";
					echo '              </tr>'."\n";
					echo '              <tr>'."\n";
		...
		{
			echo '  <tr'._bgColor(1).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sc(),(($settings['download_view'] == 0) ? '6' : $themes[get_theme()]['per_row'])).'>'._sut(sprintf($titanium_lang_new[$titanium_module_name]['SEARCH_RESULTS_NONE'],$query)).'</td>'."\n";
			echo '  </tr>'."\n";
			echo '  <tr'._bgColor(2).'>'."\n";
