...
{
	global $pnt_db, $admin_file, $lang_new, $pnt_module, $settings, $themes, $admin, $pnt_user;
	OpenTable();
	echo '<br />';
	_index_navigation_header();
	$cid 			 = (isset($_GET['cid'])) ? $_GET['cid'] : 0;
	$count_downloads = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_ITEMS."` WHERE `cid` = '".$cid."' && `isbroken` = 0 && `isactive` = 1 && `isapproved` = 1 && `isbroken` = 0"));
//-------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
	...
	$result 	= $pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_CATEGORIES."` WHERE `parentid`='".$cid."' ORDER BY `cname`, `parentid` DESC");	
	if($pnt_db->sql_numrows($result) > 0)
	{
		...
		{
			$cat_downloads = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_ITEMS."` WHERE `cid`='".$c['cid']."' && `isactive` > 0 && `isbroken` = 0"));
			if ($count == 1) 
				echo '  <tr'._bgColor(1).'>'."\n";
			...
			echo '        </tr>'."\n";
			$sresult = $pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_CATEGORIES."` WHERE `parentid`='".$c['cid']."' ORDER BY `cname`");
			while($s = $pnt_db->sql_fetchrow($sresult))
			{
				$scat_downloads = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_ITEMS."` WHERE `cid`='".$s['cid']."' && `isactive` > 0 && `isbroken` = 0"));
				echo '  <tr>'."\n";
	...
	$dresult = $pnt_db->sql_query("SELECT * FROM `"._FILE_REPOSITORY_ITEMS."` WHERE `isactive`='1' && `isbroken`='0' && `cid`='".$cid."' LIMIT ".$limit1.", ".$limit2);
	if($pnt_db->sql_numrows($dresult) > 0)
	{
		echo '<br />';
		echo '<a name="'.$cid.'"></a>';
		...
		if($settings['download_view'] == 0)
		{
			...
			echo '  </tr>'."\n";
			$items = 0;
			while($d = $pnt_db->sql_fetchrow($dresult))
			{
				$v 			= (($d['version']) ? sprintf($lang_new[$pnt_module]['V'],$d['version']) : '');
				$iteminfo 	= _collect_iteminfo($d['did']);
				$screen[$items] = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT `filename`, `title` FROM `"._FILE_REPOSITORY_SCREENSHOTS."` WHERE `did`='".$d['did']."' ORDER BY RAND()"));
				echo '  <tr'._bgColor(1).'>'."\n";
