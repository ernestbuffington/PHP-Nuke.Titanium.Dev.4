...
{
	global $titanium_db, $admin_file, $titanium_lang_new, $titanium_module_name, $settings, $admin, $titanium_user, $userinfo;
	OpenTable();
...
//------------------------------------------------------------------------------------------------
	...
	if($iteminfo['isactive'] == 0)
	{
		...
		echo '  </tr>'."\n".'<tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss(false,'center',_sc(),3).'>'._sut($titanium_lang_new[$titanium_module_name]['NOT_ACTIVE']).'</td>'."\n";
		echo '  </tr>'."\n".'<tr'._bgColor(2).'>'."\n";
...
//------------------------------------------------------------------------------------------------
		...
		echo '<br />';
		echo '<form action="modules.php?name='.$titanium_module_name.'&amp;action='.(($iteminfo['filecount'] > 1) ? 'viewfiles&amp;did='.$did : 'gogetit&amp;fid='.$iteminfo['fid']).'" method="POST">'."\n";
		echo '<a name="'.$did.'"></a>';
		echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="forumline" style="table-layout: fixed;">'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,false,_sh(),$colspan).'>'."\n";
		echo '      <span style="float: left;">'._category_parents_name($iteminfo['cid'],$iteminfo['title'],$did,$iteminfo['version'],$iteminfo['color']).((is_admin($admin)) ? '&nbsp;( <a'._ls().' href="'.$admin_file.'.php?op=file_repository&amp;action=editfile&amp;did='.$did.'">'._suh($titanium_lang_new[$titanium_module_name]['MODIFY_ITEM']).'</a> )' : '').'</span>'."\n";
		if($iteminfo['preview'])
		echo '      <span style="float: right;"><a'._ls().' href="'.$iteminfo['preview'].'" target="_blank">'._suh($titanium_lang_new[$titanium_module_name]['PREVIEW']).'</a></span>'."\n";
		echo '    </td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		if($screenshots['count'])
		echo '    <td'._tdcss('20%','center',_sc(),false,false,5).' valign="top">'.$screenshots['carousel'].'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['AUTHOR']).'&nbsp;:&nbsp;'.$iteminfo['author_www'].'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['AUTHOR_EMAIL']).'&nbsp;:&nbsp;'.((!$iteminfo['author_email']) ? $titanium_lang_new[$titanium_module_name]['NA'] : $iteminfo['author_email']).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['FILE_SIZE']).'&nbsp;:&nbsp;'.$iteminfo['filesize'].'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'.$ustring.'&nbsp;:&nbsp;'.$iteminfo['isupdated'].'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['VIEWS']).'&nbsp;:&nbsp;'.$iteminfo['views'].'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['DOWNLOADS']).'&nbsp;:&nbsp;'.$iteminfo['hits'].'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['POSTS_NEEDED']).'&nbsp;:&nbsp;'.$iteminfo['posts'].'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['WHOCAN']).'&nbsp;:&nbsp;'.$iteminfo['whocan'].'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['FILE_VERSION']).'&nbsp;:&nbsp;'.(($iteminfo['version']) ? $iteminfo['version'] : $titanium_lang_new[$titanium_module_name]['NA']).'</td>'."\n";
		echo '    <td'._tdcss($width,false,_sc()).'>'._sut($titanium_lang_new[$titanium_module_name]['RATINGS']).'&nbsp;:&nbsp;'._average_rating($did) .'&nbsp;<a href="modules.php?name='.$titanium_module_name.'&amp;action=comments&amp;did='.$did.'"><span class="dm-sprite rate-button"></span></a></td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		if($iteminfo['fixes'] && $settings['developer_mode'] == true)
		{
			echo '    <td'._tdcss(false,false,_sh(),1).'><span class="toggle-description" style="cursor: pointer; text-decoration: underline">'._suh($titanium_lang_new[$titanium_module_name]['DESCRIPTION']).'</span></td>'."\n";
			echo '    <td'._tdcss(false,false,_sh(),(($screenshots['count'] == 0) ? 1 : 2)).'><span class="toggle-fixes" style="cursor: pointer;">'._suh($titanium_lang_new[$titanium_module_name]['FIXES']).'</span></td>'."\n";
		} else {
			echo '    <td'._tdcss(false,false,_sh(),$colspan).'>'._suh($titanium_lang_new[$titanium_module_name]['DESCRIPTION']).'</td>'."\n";
		}
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).' class="item-description">'."\n";	
		echo '    <td'._tdcss(false,false,_sc(),$colspan).'>'.(($iteminfo['description']) ? _decode_bbcode_text(stripslashes($iteminfo['description']),$settings['parse_smilies']) : $titanium_lang_new[$titanium_module_name]['DESCRIPTION_NONE']).'</td>'."\n";
		echo '  </tr>'."\n";
		if($iteminfo['fixes'] && $settings['developer_mode'] == true)
		{
			echo '  <tr'._bgColor(1).' class="item-fixes" style="display: none;">'."\n";	
			echo '    <td'._tdcss(false,false,_sc(),$colspan).'>'.(($iteminfo['fixes']) ? _decode_bbcode_text(stripslashes($iteminfo['fixes']),$settings['parse_smilies']) : $titanium_lang_new[$titanium_module_name]['DESCRIPTION_NONE']).'</td>'."\n";
			echo '  </tr>'."\n";
		}
//---------------------------------------------------------------------
		if($userinfo['user_posts'] <= $iteminfo['posts'] && $iteminfo['posts'] <> 0)
		{
			...
			echo '  </tr>'."\n";
			echo '  <tr'._bgColor(1).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sc(),3).'>'.sprintf($titanium_lang_new[$titanium_module_name]['POSTS_REQUIRED'],$userinfo['user_posts'],$iteminfo['posts']).'</td>'."\n";
			echo '  </tr>'."\n";
		}
		...
		{
			echo '  <tr'._bgColor(2).'>'."\n";	
			echo '    <td'._tdcss(false,'center',_sh(),$colspan).'>'._sut(sprintf($titanium_lang_new[$titanium_module_name]['DOWNLOAD_ITEM'],$iteminfo['title_colored'])).'</td>'."\n";
			echo '  </tr>'."\n";
			if(_check_users_permissions($iteminfo['groups']) == true && $settings['usegfxcheck'] == true)
			{
				echo '  <tr'._bgColor(1).'>'."\n";	
				echo '    <td'._tdcss(false,'right',_sc(),1).'>'._sut($titanium_lang_new[$titanium_module_name]['SECURITY_CODE']).'</td>'."\n";
				...
				if (!defined('NUKE_EVO'))
				{
					...
					echo '  </tr>'."\n";
					echo '  <tr'._bgColor(1).'>'."\n";	
					echo '    <td'._tdcss(false,'right',_sc(),1).'>'._sut($titanium_lang_new[$titanium_module_name]['SECURITY_TYPE']).'</td>'."\n";
			...
			{
				echo '  <tr'._bgColor(1).'>'."\n";	
				echo '    <td'._tdcss(false,'center',_sc(),$colspan).'>'._sut(sprintf($titanium_lang_new[$titanium_module_name]['RESTRICTED'],$iteminfo['whocan'])).'</td>'."\n";
				echo '  </tr>'."\n";
			}
		}
//---------------------------------------------------------------------
		...
		{
			$submitButton = _submit(sprintf($titanium_lang_new[$titanium_module_name]['DOWNLOAD_ITEM'],$iteminfo['title']));
		}
		...
		if($iteminfo['comments'] > 0)
		{
			...
			echo '  <tr'._bgColor(2).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sh(),2).'>'._sut(sprintf($titanium_lang_new[$titanium_module_name]['COMMENT_FOR'],$iteminfo['title_colored'])).'</td>'."\n";
			echo '  </tr>'."\n";
			...
			while($c = $titanium_db->sql_fetchrow($result))
			{
				...
				echo '        </tr>'."\n";
				echo '        <tr'._bgColor(1).'>'."\n";
				echo '          <td'._tdcss(false,'right',false,false,true).'>'._sut($titanium_lang_new[$titanium_module_name]['RATING']).'&nbsp;:&nbsp;'.$c['rating'].'</td>';
				echo '        </tr>'."\n";
				echo '      </table>'."\n";
				echo '    </td>'."\n";
	...
	elseif($iteminfo['isbroken'] == true)
	{
		...
		echo '    <td'._tdcss(false,'center',_sh(),3).'>'._suh($titanium_lang_new[$titanium_module_name]['ERROR']).'</td>'."\n";
		echo '  </tr>'."\n".'<tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss(false,'center',_sc(),3).'>'._sut($titanium_lang_new[$titanium_module_name]['ERROR_DOWNLOAD_MISSING']).'</td>'."\n";
		echo '  </tr>'."\n".'<tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,'center',_sf(),3).'>&nbsp;</td>'."\n";
		echo '  </tr>'."\n";
		echo '</table>'."\n";
...
{
	global $titanium_db, $titanium_lang_new, $titanium_module_name, $settings, $admin;
	$did = _escape_string($_GET['did']);
	$iteminfo 	 = _collect_iteminfo($did);
	...
	{		
		...
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,'center',_sf(),3).'>[ <a'._ls().' href="modules.php?name=File_Repository&action=view&did='.$iteminfo['did'].'">'.$titanium_lang_new[$titanium_module_name]['GOBACK'].'</a> ]</td>'."\n"; // '._submit($titanium_lang_new[$titanium_module_name]['DOWNLOAD']).'
		echo '  </tr>'."\n";
		echo '</table>'."\n";
	}
	else
	{
		...
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>';
		echo '    <td'._tdcss('70%',false,_sh()).'>'._sut($titanium_lang_new[$titanium_module_name]['FILE_TITLE']).'</td>';
		echo '    <td'._tdcss('15%','center',_sh()).'>'._sut($titanium_lang_new[$titanium_module_name]['FILE_SIZE']).'</td>';
		echo '    <td'._tdcss('15%','center',_sh()).'>'._sut($titanium_lang_new[$titanium_module_name]['DOWNLOAD']).'</td>';
		echo '  </tr>';
		while( $f = $titanium_db->sql_fetchrow($result) )
		{
			...
			echo '    <td'._tdcss('15%','center',_sc()).'><a'._ls().' href="modules.php?name='.$titanium_module_name.'&amp;action=gogetit&amp;fid='.$f['fid'].'">Download</a></td>';
			echo '  </tr>';			
		}
		$titanium_db->sql_freeresult($result);
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,'center',_sf(),3).'>[ <a'._ls().' href="modules.php?name='.$titanium_module_name.'&action=view&did='.$iteminfo['did'].'">'.$titanium_lang_new[$titanium_module_name]['GOBACK'].'</a> ]</td>'."\n";
		echo '  </tr>'."\n";
		echo '</table>'."\n";
	}	
	CloseTable();
...
{
	global $titanium_db, $titanium_lang_new, $titanium_module_name, $settings, $do_gzip_compress, $admin;
	...
	if (!security_code_check($_POST['g-recaptcha-response'],array(0,1,2,3,4,5,6,7)) && _check_users_permissions($iteminfo['groups']) == true && $settings['usegfxcheck'] == true && !isset($_COOKIE['gfx_code-'.$iteminfo['did']]) && $adminBypass && get_evo_option('recap_site_key') && get_evo_option('recap_priv_key'))
	{
		...
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(1).'>'."\n";	
		echo '    <td'._tdcss(false,'center',_sc(),3).'>'.$titanium_lang_new[$titanium_module_name]['NOMATCH'].' - '.security_code_check($_POST['g-recaptcha-response'],array(0,1,2,3,4,5,6,7)).'</td>'."\n";
		echo '  </tr>'."\n";
		echo '  <tr'._bgColor(2).'>'."\n";
		echo '    <td'._tdcss(false,'center',_sf(),3).'>[ <a'._ls().' href="modules.php?name=File_Repository&action=view&did='.$iteminfo['did'].'">'.$titanium_lang_new[$titanium_module_name]['GOBACK'].'</a> ]</td>'."\n"; // '._submit($titanium_lang_new[$titanium_module_name]['DOWNLOAD']).'
		echo '  </tr>'."\n";
	...
	else
	{
		...
		else
		{
			...
			echo '  </tr>'."\n";
			echo '  <tr'._bgColor(1).'>'."\n";	
			echo '    <td'._tdcss(false,'center',_sc(),3).'>'._sut($titanium_lang_new[$titanium_module_name]['ERROR_DOWNLOADING']).'</td>'."\n";
			echo '  </tr>'."\n";
			echo '  <tr'._bgColor(2).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sf(),3).'>[ <a'._ls().' href="modules.php?name='.$titanium_module_name.'">'._sut($titanium_lang_new[$titanium_module_name]['GOBACK']).'</a> ]</td>'."\n";
			echo '  </tr>'."\n";
			echo '</table>'."\n";
			CloseTable();
		}
