...
if ($config['button_standard'] == 1){
    ...
    if($numrows == 0)
	{
        echo '<div align="center"><span color="red" size="3">!'.$titanium_lang_new[$titanium_module_name]['NO_ACTIVE_SITES'].'!</span></div>';
    } 
	else 
	{
        if ($numrows > 0) 
		{
		    ...
		    while(list($id, $site_name, $site_url, $site_image, $site_description, $site_hits, $site_status, $date_added) = $titanium_db->sql_fetchrow($result)) 
			{
				...
				<a href="modules.php?name='.$titanium_module_name.'&amp;op=visit&amp;id='.$id.'" target="_blank"><i class="bi bi-link"></i></i> '.set_smilies(decode_bbcode(stripslashes($site_name), 1, true)).' <i class="bi bi-link"></i></i></a></legend>';				
                ...
                print "<td rowspan=\"2\" width=\"110\" align=\"center\" valign=\"top\"><a href='modules.php?name=".$titanium_module_name."&amp;op=visit&amp;id=".$id."' target='_blank'><img height=\"31\" src='".$site_image."' ".$settings." /></a></a>";

                if (is_mod_admin($titanium_module_name)) 
				{
                echo "<br /><a href='".$admin_file.".php?op=edit_button&amp;
                id=".$id."'><img src='modules/".$titanium_module_name."/images/edit.png' border='0' 
                alt='".$titanium_lang_new[$titanium_module_name]['EDIT']."' title='".$titanium_lang_new[$titanium_module_name]['EDIT']."' /></a>";
                echo " <a href='".$admin_file.".php?op=delete_button&amp;id=".$id."'><img 
                src='modules/".$titanium_module_name."/images/delete.png' 
				border='0' alt='".$titanium_lang_new[$titanium_module_name]['DELETE']."' title='".$titanium_lang_new[$titanium_module_name]['DELETE']."' /></a>";
                }

               print '</td><td align="left" valign="top" height="3" width="80%"><strong>&nbsp;<i class="bi bi-calendar2-check"></i>&nbsp; '.$titanium_lang_new[$titanium_module_name]['ADDED'].' '.formatTimestamp($date_added).'</strong></td>';
               print '</tr>';
               print '<tr>';
               
			   print '<td align="left" valign="top" height="3" width="0">';
			   print '<strong>&nbsp;<i class="bi bi-eye"></i>&nbsp; '.$titanium_lang_new[$titanium_module_name]['VISITS'].': '.$site_hits.'</strong></td>';
			   print '</tr>';
...
if ($config['button_banner'] == 1){
    ...
    if ($numrows > 0) {
        ...
        while(list($id, $site_name, $site_url, $site_image, $site_description, $site_hits, $site_status, $date_added) = $titanium_db->sql_fetchrow($result)) {
            ...
            echo "<a href='modules.php?name=".$titanium_module_name."&amp;op=visit&amp;id=$id' target='_blank'><img src='".$site_image."' ".$settings." /></a>";
            if (is_mod_admin($titanium_module_name)) {
...
if ($config['button_resource'] == 1){
    ...
    if ($numrows > 0) {
        ...
        while(list($id, $site_name, $site_url, $site_image, $site_description, $site_hits, $site_status, $date_added) = $titanium_db->sql_fetchrow($result)) {
            ...
            echo "<a href='modules.php?name=".$titanium_module_name."&amp;op=visit&amp;id=$id' target='_blank'><img src='".$site_image."' ".$settings." align='absmiddle' /></a>";
            if (is_mod_admin($titanium_module_name)) {
                echo "<br /><br /><a href='".$admin_file.".php?op=edit_button&amp;id=".$id."'><img src='modules/".$titanium_module_name."/images/edit.png' border='0' alt='".$titanium_lang_new[$titanium_module_name]['EDIT']."' title='".$titanium_lang_new[$titanium_module_name]['EDIT']."' /></a>";
                echo " <a href='".$admin_file.".php?op=delete_button&amp;id=".$id."'><img src='modules/".$titanium_module_name."/images/delete.png' border='0' alt='".$titanium_lang_new[$titanium_module_name]['DELETE']."' title='".$titanium_lang_new[$titanium_module_name]['DELETE']."' /></a>";
            }
