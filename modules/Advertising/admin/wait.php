...
global $admin_file, $pnt_db, $pnt_prefix, $banners, $cache;
...
if($banners && is_mod_admin('Advertising')) {
    ...
    if (!$active = $cache->load('numbanact', 'submissions')) {
        list($active) = $pnt_db->sql_ufetchrow("SELECT COUNT(*) FROM " . $pnt_prefix . "_banner WHERE active='1'", SQL_NUM);
        $cache->save('numbanact', 'submissions', $active);
    }
    if (!$inactive = $cache->load('numbandea', 'submissions')) {
        list($inactive) = $pnt_db->sql_ufetchrow("SELECT COUNT(*) FROM " . $pnt_prefix . "_banner WHERE active='0'", SQL_NUM);
