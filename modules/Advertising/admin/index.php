...
if(is_mod_admin($pnt_module)) {
    ...
    list($c_num) = $pnt_db->sql_ufetchrow("SELECT COUNT(*) FROM ".$pnt_prefix."_banner_clients", SQL_NUM);
    if ($c_num == 0) {
    ...
    function BannersAdmin() {
        global $pnt_prefix, $pnt_db, $bgcolor2, $banners, $admin_file, $ad_admin_menu_main, $bgcolor1;
        ...
        $result = $pnt_db->sql_query("SELECT bid, cid, name, imptotal, impmade, clicks, imageurl, date, position, active, ad_class, type FROM " . $pnt_prefix . "_banner WHERE active='1' ORDER BY position,bid", true);
        while (list($bid, $cid, $name, $imptotal, $impmade, $clicks, $imageurl, $date, $type, $active, $ad_class) = $pnt_db->sql_fetchrow($result)) {
            $bid = intval($bid);
            $cid = intval($cid);
            ...
            list($cid, $client_name) = $pnt_db->sql_ufetchrow("SELECT cid, name FROM " . $pnt_prefix . "_banner_clients WHERE cid='$cid'");
            $cid = intval($cid);
            $name = trim($name);
            if (empty($name)) {
                $name = _NONE;
            ...
            $row2 = $pnt_db->sql_ufetchrow("SELECT apid, position_name FROM ".$pnt_prefix."_banner_positions WHERE position_number='$type'");
        ...
        ."<td align=\"center\"><strong>" . _CLASS . "</strong></td>"
        ."<td align=\"center\"><strong>" . _FUNCTIONS . "</strong></td><tr>";
        $result = $pnt_db->sql_query("SELECT bid, cid, name, imptotal, impmade, clicks, imageurl, date, position, active, ad_class FROM " . $pnt_prefix . "_banner WHERE active='0' ORDER BY position,bid");
        while ($row = $pnt_db->sql_fetchrow($result)) {
            ...
            $row2 = $pnt_db->sql_ufetchrow("SELECT cid, name FROM " . $pnt_prefix . "_banner_clients WHERE cid='$cid'");
            $cid = intval($row2['cid']);
            $name = trim($row2['name']);
        ...
        ."<td align=\"center\"><strong>" . _FUNCTIONS . "</strong></td><tr>";
        $result3 = $pnt_db->sql_query("SELECT cid, name, contact, email FROM " . $pnt_prefix . "_banner_clients ORDER BY cid");
        while ($row3 = $pnt_db->sql_fetchrow($result3)) {
            $cid = intval($row3['cid']);
            $name = $row3['name'];
            $contact = $row3['contact'];
            $email = $row3['email'];
            $result4 = $pnt_db->sql_query("SELECT cid FROM " . $pnt_prefix . "_banner WHERE cid='$cid' AND active='1'");
            $numrows = $pnt_db->sql_numrows($result4);
            $row4 = $pnt_db->sql_fetchrow($result4);
            $pnt_db->sql_freeresult($result4);
            $rcid = intval($row4['cid']);
            list($numrows2) = $pnt_db->sql_ufetchrow("SELECT COUNT(*) FROM " . $pnt_prefix . "_banner WHERE cid='$cid' AND active='0'");
            echo "<td bgcolor=\"$bgcolor1\" align=\"center\">$name</td>"
    ...
    function add_banner() {
        global $pnt_prefix, $pnt_db, $banners, $admin_file, $ad_admin_menu;
        ...
        echo "<br />";
        OpenTable();
        $result = $pnt_db->sql_query("select * FROM ".$pnt_prefix."_banner_clients");
        if($pnt_db->sql_numrows($result) > 0) {
            ...
            $result = $pnt_db->sql_query("SELECT position_number, position_name FROM ".$pnt_prefix."_banner_positions ORDER BY position_number");
            while ($row = $pnt_db->sql_fetchrow($result)) {
    ...
    function BannerStatus($bid, $status) {
        ...
        $bid = intval($bid);
        $pnt_db->sql_query("UPDATE " . $pnt_prefix . "_banner SET active='$active' WHERE bid='$bid'");
    ...
    function BannersAdd($name, $cid, $adname, $imptotal, $imageurl, $clickurl, $alttext, $position, $active, $ad_class, $ad_code, $ad_width, $ad_height) {
        global $pnt_prefix, $pnt_db, $admin_file, $ad_admin_menu;
        ...
        $pnt_db->sql_query("insert into " . $pnt_prefix . "_banner VALUES (NULL, '$cid', '$adname', '$imptotal', '1', '0', '$imageurl', '$clickurl', '$alttext', now(), '0000-00-00 00:00:00', '$position', '$active', '$ad_class', '$ad_code', '$ad_width', '$ad_height', '')");
        redirect_titanium($admin_file.'.php?op=BannersAdmin');
    }

    function BannerAddClient($name, $contact, $email, $login, $passwd, $extrainfo) {
        global $pnt_prefix, $pnt_db, $admin_file;
        $pnt_db->sql_query("insert into " . $pnt_prefix . "_banner_clients VALUES (NULL, '$name', '$contact', '$email', '$login', '$passwd', '$extrainfo')");
        redirect_titanium($admin_file.'.php?op=BannersAdmin');
    }

    function BannerDelete($bid, $ok=0) {
        global $pnt_prefix, $pnt_db, $admin_file, $bgcolor1, $bgcolor2, $ad_admin_menu;
        $bid = intval($bid);
        if ($ok == 1) {
            $pnt_db->sql_query("DELETE FROM " . $pnt_prefix . "_banner WHERE bid='$bid'");
            redirect_titanium($admin_file.".php?op=BannersAdmin");
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
            OpenTable();
	    ...
	    echo "<br />";
            ...
            CloseTable();
            echo "<br />";
            $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT cid, name, imptotal, impmade, clicks, imageurl, clickurl, ad_class, ad_code, ad_width, ad_height FROM " . $pnt_prefix . "_banner WHERE bid='$bid'"));
            $cid = intval($row['cid']);
            $imptotal = intval($row['imptotal']);
            $impmade = intval($row['impmade']);
            $clicks = intval($row['clicks']);
    ...
    function BannerEdit($bid) {
        global $pnt_prefix, $pnt_db, $admin_file, $ad_admin_menu, $admlang;
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
	    echo "<div align=\"center\">\n<a href=\"$admin_file.php?op=BannersAdmin\">" . _BANNERS_ADMIN_HEADER . "</a></div>\n";
        echo "<br /><br />";
        ...
        echo "<br />";
        $bid = intval($bid);
        $row = $pnt_db->sql_ufetchrow("SELECT cid, name, imptotal, impmade, clicks, imageurl, clickurl, alttext, date, position, active, ad_class, ad_code, ad_width, ad_height FROM " . $pnt_prefix . "_banner WHERE bid='$bid'");
        $cid = intval($row['cid']);
        $imptotal = intval($row['imptotal']);
        $impmade = intval($row['impmade']);
        $clicks = intval($row['clicks']);
        ...
        $result3 = $pnt_db->sql_query("SELECT cid, name FROM " . $pnt_prefix . "_banner_clients");
        while ($row3 = $pnt_db->sql_fetchrow($result3)) {
        ...
        $result4 = $pnt_db->sql_query("SELECT position_number, position_name FROM ".$pnt_prefix."_banner_positions ORDER BY position_number");
    ...
    function BannerChange($bid, $cid, $adname, $imptotal, $impadded, $imageurl, $clickurl, $alttext, $position, $active, $ad_code, $ad_width, $ad_height, $impmade) {
        ...
        $active = intval($active);
        $bid = intval($bid);
        $pnt_db->sql_query("UPDATE " . $pnt_prefix . "_banner SET cid='$cid', name='$adname', imptotal='$imp', imageurl='$imageurl', clickurl='$clickurl', alttext='$alttext', position='$position', active='$active', ad_code='$ad_code', ad_width='$ad_width', ad_height='$ad_height' WHERE bid='$bid'");
        redirect_titanium($admin_file.".php?op=BannersAdmin");
    }
    ...
    function BannerClientDelete($cid, $ok=0) {
        ...
        $cid = intval($cid);
        if ($ok==1) {
            $pnt_db->sql_query("DELETE FROM " . $pnt_prefix . "_banner WHERE cid='$cid'");
            $pnt_db->sql_query("DELETE FROM " . $pnt_prefix . "_banner_clients WHERE cid='$cid'");
            redirect_titanium($admin_file.".php?op=BannersAdmin");
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
            OpenTable();
	    ...
	    echo "<br />";
            ...
            CloseTable();
            echo "<br />";
            $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT cid, name FROM " . $pnt_prefix . "_banner_clients WHERE cid='$cid'"));
            $cid = intval($row['cid']);
            $name = $row['name'];
            OpenTable();
            echo "<center><strong>" . _DELETECLIENT . ": $name</strong><br /><br />
                " . _SURETODELCLIENT . "<br /><br />";
            $result2 = $pnt_db->sql_query("SELECT imageurl, clickurl FROM " . $pnt_prefix . "_banner WHERE cid='$cid'");
    ...
    function BannerClientEdit($cid) {
        ...
        echo "<br />";
        $cid = intval($cid);
        $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT name, contact, email, login, passwd, extrainfo FROM " . $pnt_prefix . "_banner_clients WHERE cid='$cid'"));
        $name = $row['name'];
        $contact = $row['contact'];
        $email = $row['email'];
        $login = $row['login'];
    ...
    function BannerClientChange($cid, $name, $contact, $email, $extrainfo, $login, $passwd) {
        ...
        $cid = intval($cid);
        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner_clients SET name='$name', contact='$contact', email='$email', login='$login', passwd='$passwd', extrainfo='$extrainfo' WHERE cid='$cid'");
        redirect_titanium($admin_file.".php?op=BannersAdmin#top");
    }

    function ad_positions() {
        global $pnt_prefix, $pnt_db, $banners, $admin_file, $ad_admin_menu, $bgcolor1, $bgcolor2;
        ...
        $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions ORDER BY apid");
        while ($row = $pnt_db->sql_fetchrow($result)) {
            $ban_num = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'"));
        ...
        echo "<br />";
        OpenTable();
        $numrows = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions"));
        if ($numrows == 0) {
            $pos_num = 0;
        } else {
            $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT position_number FROM ".$pnt_prefix."_banner_positions ORDER BY position_number DESC LIMIT 0,1"));
    ...
    function position_save($apid=0, $ad_position_number, $ad_position_name, $position_new=0) {
        ...
        if ($position_new == 1) {
            $pnt_db->sql_query("INSERT INTO ".$pnt_prefix."_banner_positions VALUES (NULL, '$ad_position_number', '$ad_position_name')");
        } else {
            $apid = intval($apid);
            $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner_positions SET position_name='$ad_position_name' WHERE apid='$apid'");
        }
    ...
    function position_edit($apid) {
        ...
        echo "<br />";
        OpenTable();
        $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'"));
    ...
    function position_delete($apid, $ok=0, $active=0, $new_pos=x) {
        ...
        if ($ok == 1) {
            if ($new_pos == "x" || empty($new_post)) {
                $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'");
            } else {
                if ($active == "same") {
                    $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'"));
                    $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $pnt_db->sql_fetchrow($result)) {
                        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner SET position='$new_pos' WHERE bid='".$row2['bid']."'");
                    }
                    $pnt_db->sql_freeresult($result);
                    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'");
                    ...
                    $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $pnt_db->sql_fetchrow($result)) {
                        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner SET position='$new_pos', active='1' WHERE bid='".$row2['bid']."'");
                    }
                    $pnt_db->sql_freeresult($result);
                    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'");
                    ...
                    $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'");
                    while($row2 = $pnt_db->sql_fetchrow($result)) {
                        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner SET position='$new_pos', active='0' WHERE bid='".$row2['bid']."'");
                    }
                    $pnt_db->sql_freeresult($result);
                    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'");
                } elseif ($active == "delete_all") {
                    $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'"));
                    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'");
                    $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_positions WHERE apid='$apid'");
                }
            }
            redirect_titanium($admin_file.'.php?op=ad_positions');
	    ...
	    echo "<br />";
            ...
            $numrows = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner WHERE position='".$row['position_number']."'"));
            if($numrows != 0) {
                echo ""._POSITIONHASADS."<br /><br />";
                echo "<form action=\"".$admin_file.".php\" method=\"POST\">";
                echo ""._MOVEADS.": <select name=\"new_pos\">";
                $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_positions WHERE apid!='$apid'");
                while($row = $pnt_db->sql_fetchrow($result)) {
    ...
    function ad_terms($save=0, $terms_body=0, $country=0) {
        global $pnt_prefix, $pnt_db, $banners, $admin_file, $ad_admin_menu, $admlang;
        if ($save != 0) {
            $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner_terms SET terms_body='".Fix_Quotes($terms_body)."', country='$country'");
        ...
        $result = $pnt_db->sql_query("SELECT `flag_name` FROM `".$pnt_prefix."_bbflags` ORDER BY `flag_name`");
        while ($row2 = $pnt_db->sql_fetchrow($result)) {
    ...
    function ad_plans() {
        global $pnt_prefix, $pnt_db, $admin_file, $ad_admin_menu, $bgcolor1, $bgcolor2;
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
	    echo "<div align=\"center\">\n<a href=\"$admin_file.php?op=BannersAdmin\">" . _BANNERS_ADMIN_HEADER . "</a></div>\n";
        echo "<br /><br />";
        ...
        CloseTable();
        echo "<br />";
        $numrows = $pnt_db->sql_numrows($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_plans"));
        if ($numrows != 0) {
            OpenTable();
            $result = $pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_plans");
    ...
    function ad_plans_add($name, $description, $delivery, $type, $price, $buy_links, $status) {
        ...
        if (!empty($name) AND !empty($description) AND !empty($delivery) AND (isset($type) AND is_numeric($type)) AND !empty($price) AND !empty($buy_links) AND !empty($status)) {
            $pnt_db->sql_query("INSERT INTO ".$pnt_prefix."_banner_plans VALUES (NULL, '$status', '$name', '$description', '$delivery', '$type', '$price', '$buy_links')");
    ...
    function ad_plans_edit($pid) {
        ...
        echo "<br />";
        OpenTable();
        $row = $pnt_db->sql_fetchrow($pnt_db->sql_query("SELECT * FROM ".$pnt_prefix."_banner_plans WHERE pid='$pid'"));
    ...
    function ad_plans_save($pid, $name, $description, $delivery, $type, $price, $buy_links, $status) {
        ...
        if (!empty($name) AND !empty($description) AND !empty($delivery) AND (isset($type) AND is_numeric($type)) AND !empty($price) AND !empty($buy_links) AND !empty($status)) {
            $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner_plans SET active='$status', name='$name', description='$description', delivery='$delivery', delivery_type='$type', buy_links='$buy_links', price='$price' WHERE pid='$pid'");
            redirect_titanium($admin_file.'.php?op=ad_plans');
            exit;
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
    ...
    function ad_plans_delete($pid, $ok=0) {
        ...
        if ($ok == 1) {
            $pnt_db->sql_query("DELETE FROM ".$pnt_prefix."_banner_plans WHERE pid='$pid'");
            redirect_titanium($admin_file.'.php?op=ad_plans');
            exit;
        } else {
            include_once(NUKE_BASE_DIR.'header.php');
    ...
    function ad_plans_status($pid, $status) {
        ...
        $pid = intval($pid);
        $pnt_db->sql_query("UPDATE ".$pnt_prefix."_banner_plans SET active='$active' WHERE pid='$pid'");
        redirect_titanium($admin_file.'.php?op=ad_plans');
    }
