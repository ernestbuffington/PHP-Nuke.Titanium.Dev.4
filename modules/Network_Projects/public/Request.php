...
if($project['allowrequests'] > 0) {
  ...
  $memberresult = $titanium_db2->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id' ORDER BY `member_id`");
  ...
  echo "<tr><td bgcolor='$bgcolor2' width='100%'><nobr><strong>"._NETWORK_COMMENTS."</strong> <strong>(</strong> <a href='modules.php?name=$titanium_module_name&amp;op=RequestCommentSubmit&amp;request_id=$request_id'>"._NETWORK_COMMENTADD."</a> <strong>)</strong></nobr></td><tr>\n";
  if($comment_total > 0){
