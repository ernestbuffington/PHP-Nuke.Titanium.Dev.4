...
$pnt_db2->sql_query("UPDATE `".$network_prefix."_tasks` SET `project_id`='$project_id', `task_name`='$task_name', `task_description`='$task_description', `priority_id`='$priority_id', `status_id`='$status_id', `task_percent`='$task_percent', `date_started`='$phpbb2_start_date', `date_finished`='$finish_date' WHERE `task_id`='$task_id'");
...
if(implode("", $member_ids) > "") {
  while(list($null, $member_id) = each($member_ids)) {
    ...
    if($numrows == 0) {
      $pnt_db2->sql_query("INSERT INTO `".$network_prefix."_tasks_members` VALUES ('$task_id', '$member_id', '0')");        
    }
  }
}
