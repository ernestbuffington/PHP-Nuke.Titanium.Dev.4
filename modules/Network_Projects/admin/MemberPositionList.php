...
//echo "<br />\n";
$positionresult = $db2->sql_query("SELECT * FROM `".$network_prefix."_members_positions` WHERE `position_weight` > 0 ORDER BY `position_weight`");
$position_total = $db2->sql_numrows($positionresult);
OpenTable();
...
if($position_total != 0){
  while($position_row = $db2->sql_fetchrow($positionresult)) {
    ...
    list($pid1) = $db2->sql_fetchrow($db2->sql_query("SELECT `position_id` FROM `".$network_prefix."_members_positions` WHERE `position_weight`='$weight1'"));
    list($pid2) = $db2->sql_fetchrow($db2->sql_query("SELECT `position_id` FROM `".$network_prefix."_members_positions` WHERE `position_weight`='$weight3'"));
