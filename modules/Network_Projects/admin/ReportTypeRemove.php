...
$typeresult = $pnt_db2->sql_query("SELECT `type_name` FROM `".$network_prefix."_reports_types` WHERE `type_id`='$type_id'");
...
$typelist = $pnt_db2->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_reports_types` WHERE `type_id` != '$type_id' AND `type_id` > 0 ORDER BY `type_weight`");
