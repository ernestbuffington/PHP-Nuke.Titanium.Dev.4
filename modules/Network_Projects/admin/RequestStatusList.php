...
//echo "<br />";
$statusresult = $db2->sql_query("SELECT * FROM `".$network_prefix."_requests_status` WHERE `status_weight` > 0 ORDER BY `status_weight`");
...
if($status_total != 0){
  while($status_row = $db2->sql_fetchrow($statusresult)) {
    ...
    list($pid1) = $db2->sql_fetchrow($db2->sql_query("SELECT `status_id` FROM `".$network_prefix."_requests_status` WHERE `status_weight`='$weight1'"));
    list($pid2) = $db2->sql_fetchrow($db2->sql_query("SELECT `status_id` FROM `".$network_prefix."_requests_status` WHERE `status_weight`='$weight3'"));
