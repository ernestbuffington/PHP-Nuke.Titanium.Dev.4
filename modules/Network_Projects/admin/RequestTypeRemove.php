...
$typeresult = $pnt_db2->sql_query("SELECT `type_name` FROM `".$network_prefix."_requests_types` WHERE `type_id`='$type_id'");
list($type_name) = $pnt_db2->sql_fetchrow($statusresult);
...
$typelist = $pnt_db2->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_requests_types` WHERE `type_id` != '$type_id' AND `type_id` > 0 ORDER BY `type_weight`");
