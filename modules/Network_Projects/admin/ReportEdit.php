...
$pagetitle = _NETWORK_TITLE.' v'.$pj_config['version_number'].' - '._NETWORK_REPORTS.': '._NETWORK_REPORTEDIT;
...
while(list($p_project_id, $p_project_name) = $pnt_db2->sql_fetchrow($projectlist)){
...
$typelist = $pnt_db2->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_reports_types` ORDER BY `type_weight`");
while(list($t_type_id, $t_type_name) = $pnt_db2->sql_fetchrow($typelist)){
...
$statuslist = $pnt_db2->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_reports_status` ORDER BY `status_weight`");
...
while(list($member_id, $member_name) = $pnt_db2->sql_fetchrow($memberlistresult)) {
    $memberexresult = $pnt_db2->sql_query("SELECT `member_id` FROM `".$network_prefix."_reports_members` WHERE `member_id`='$member_id' AND `report_id`='$report_id'");
...
$membersresult = $pnt_db2->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_reports_members` WHERE `report_id`='$report_id'");
$memrows = $pnt_db2->sql_numrows($membersresult);
if($memrows > 0){
