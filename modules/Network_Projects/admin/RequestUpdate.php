...
$titanium_db2->sql_query("UPDATE `".$network_prefix."_requests` SET `project_id`='$project_id', `type_id`='$type_id', `request_name`='$request_name', `request_description`='$request_description', `submitter_name`='$submitter_name', `submitter_email`='$submitter_email', `status_id`='$status_id', `date_modified`='$date' WHERE `request_id`='$request_id'");
...
if(implode("", $member_ids) > "") {
  while(list($null, $member_id) = each($member_ids)) {
    $numrows = $titanium_db2->sql_numrows($titanium_db2->sql_query("SELECT * FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id' AND `member_id`='$member_id'"));
    if($numrows == 0) {
      $titanium_db2->sql_query("INSERT INTO `".$network_prefix."_requests_members` VALUES ('$request_id', '$member_id', '".$pj_config['new_request_position']."')");
    }
  }
}
