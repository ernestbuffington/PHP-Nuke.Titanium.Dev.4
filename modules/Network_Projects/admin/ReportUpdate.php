...
$pnt_db2->sql_query("UPDATE `".$network_prefix."_reports` SET `project_id`='$project_id', `type_id`='$type_id', `report_name`='$report_name', `report_description`='$report_description', `submitter_name`='$submitter_name', `submitter_email`='$submitter_email', `status_id`='$status_id', `date_modified`='$date' WHERE `report_id`='$report_id'");
$pnt_db2->sql_query("OPTIMIZE TABLE `".$network_prefix."_reports`");
if(implode("", $member_ids) > "") {
  while(list($null, $member_id) = each($member_ids)) {
    $numrows = $pnt_db2->sql_numrows($pnt_db2->sql_query("SELECT * FROM `".$network_prefix."_reports_members` WHERE `report_id`='$report_id' AND `member_id`='$member_id'"));
    if($numrows == 0) {
      $pnt_db2->sql_query("INSERT INTO `".$network_prefix."_reports_members` VALUES ('$report_id', '$member_id', '".$pj_config['new_report_position']."')");
    }
  }
}
list($submitter_email) = $pnt_db2->sql_fetchrow($pnt_db2->sql_query("SELECT `submitter_email` FROM `".$network_prefix."_reports` WHERE `report_id`='$report_id'"));
