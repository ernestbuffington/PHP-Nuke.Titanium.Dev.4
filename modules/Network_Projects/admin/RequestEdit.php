...
$typelist = $pnt_db2->sql_query("SELECT `type_id`, `type_name` FROM `".$network_prefix."_requests_types` ORDER BY `type_weight`");
...
$statuslist = $pnt_db2->sql_query("SELECT `status_id`, `status_name` FROM `".$network_prefix."_requests_status` ORDER BY `status_weight`");
...
while(list($member_id, $member_name) = $pnt_db2->sql_fetchrow($memberlistresult)) {
    $memberexresult = $pnt_db2->sql_query("SELECT `member_id` FROM `".$network_prefix."_requests_members` WHERE `member_id`='$member_id' AND `request_id`='$request_id'");
...
$membersresult = $pnt_db2->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_requests_members` WHERE `request_id`='$request_id'");
