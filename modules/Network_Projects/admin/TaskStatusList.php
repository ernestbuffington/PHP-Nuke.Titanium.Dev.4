...
$pagetitle = _NETWORK_TITLE.' v'.$pj_config['version_number'].' - '._NETWORK_TASKS.': '._NETWORK_STATUSLIST;
include_once(NUKE_BASE_DIR.'header.php');

pjadmin_menu(_NETWORK_TASKS.': '._NETWORK_STATUSLIST);
//echo "<br />\n";
$statusresult = $pnt_db2->sql_query("SELECT * FROM `".$network_prefix."_tasks_status` WHERE `status_weight` > 0 ORDER BY `status_weight`");
...
if($status_total != 0){
  while($status_row = $pnt_db2->sql_fetchrow($statusresult)) {
    ...
    list($pid1) = $pnt_db2->sql_fetchrow($pnt_db2->sql_query("SELECT `status_id` FROM `".$network_prefix."_tasks_status` WHERE `status_weight`='$weight1'"));
    list($pid2) = $pnt_db2->sql_fetchrow($pnt_db2->sql_query("SELECT `status_id` FROM `".$network_prefix."_tasks_status` WHERE `status_weight`='$weight3'"));
    echo "<td align='center'><nobr>";
    if($pid1 AND $pid1 > 0) {
      echo "<a href='".$admin_file.".php?op=TaskStatusOrder&amp;weight=".$status_row['status_weight']."&amp;pid=".$status_row['status_id']."&amp;weightrep=$weight1&amp;pidrep=$pid1'><img src='modules/$pnt_module/images/weight_up.png' border='0' hspace='3' alt='"._NETWORK_UP."' title='"._NETWORK_UP."'></a>";
    } else {
      echo "<img src='modules/$pnt_module/images/weight_up_no.png' border='0' hspace='3' alt='' title=''>";
    }
    if($pid2) {
      echo "<a href='".$admin_file.".php?op=TaskStatusOrder&amp;weight=".$status_row['status_weight']."&amp;pid=".$status_row['status_id']."&amp;weightrep=$weight3&amp;pidrep=$pid2'><img src='modules/$pnt_module/images/weight_dn.png' border='0' hspace='3' alt='"._NETWORKDOWN."' title='"._NETWORK_DOWN."'></a>";
    } else {
