...
echo "<tr><td bgcolor='$bgcolor2'>"._NETWORK_PROJECT_TYPE.":</td>\n";

echo "<td><select name='featured'>\n";
echo "<option value='0' selected>PHP-Nuke Titanium Web Portal</option>"; 
echo "<option value='1'$sel2>PHP-Nuke Titanium Module</option>\n";
echo "<option value='2'$sel2>PHP-Nuke Titanium Block</option>\n";
echo "<option value='3'$sel2>PHP-Nuke Titanium phpBB2 Mod</option>\n";
echo "<option value='4'$sel2>PHP-Nuke Titanium Theme</option>\n";
echo "<option value='5'$sel2>Nuke Evolution Web Portal</option>\n";
echo "<option value='6'$sel2>Nuke Evolution Module</option>\n";
echo "<option value='7'$sel2>Nuke Evolution Block</option>\n";
echo "<option value='8'$sel2>Nuke Evolution phpBB2 Mod</option>\n";
echo "<option value='9'$sel2>Nuke Evolution Theme Mod</option>\n";
echo "<option value='10$sel2'>Misc Project</option></select>\n";
echo "</td></tr>\n";
...
{
  $memberexresult = $pnt_db2->sql_query("SELECT `member_id` FROM `".$network_prefix."_projects_members` WHERE `member_id`='$member_id' AND `project_id`='$project_id'");
  $numrows = $pnt_db2->sql_numrows($memberexresult);
...
$membersresult = $pnt_db2->sql_query("SELECT `member_id`, `position_id` FROM `".$network_prefix."_projects_members` WHERE `project_id`='$project_id'");

$numrows = $pnt_db2->sql_numrows($membersresult);

if($numrows > 0)
{
  while(list($member_id, $position_id) = $pnt_db2->sql_fetchrow($membersresult))
  {
    ...
    $positionlistresult = $pnt_db2->sql_query("SELECT `position_id`, `position_name` FROM `".$network_prefix."_members_positions` ORDER BY `position_weight`");
  
    while(list($l_position_id, $l_position_name) = $pnt_db2->sql_fetchrow($positionlistresult)) 
	{
