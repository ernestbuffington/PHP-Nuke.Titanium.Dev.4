...
//echo "<br />\n";
$taskresult = $db2->sql_query("SELECT `task_id`, `task_name`, `priority_id`, `status_id` FROM `".$network_prefix."_tasks` WHERE `project_id`='$project_id' ORDER BY `task_name`");
...
if($task_total != 0){
  while(list($task_id, $task_name, $priority_id, $status_id) = $db2->sql_fetchrow($taskresult)) {
