...
function view_display ($page=0) {
    global $gen_configs, $page_configs, $donations, $titanium_lang_donate, $dis;
    ...
    if (is_array($donations)):
	    ...
	    for ($i=$phpbb2_start; $i < $stop; $i++) {
	        ...
	        echo "<tr>\n";
	            ...
	            if ($page_configs['show_amount'] == 'yes') {
	                ...
	                echo "<td><div align=\"center\">".$titanium_lang_donate['N/A']."</div></td>\n";
	            }
	    ...
	    if ($page != '0') {
	        $prev_page = $page - 1;
	        $prev = '<strong><a href="modules.php?name=Donations&op=view&page='.$prev_page.'">'.$titanium_lang_donate['PREV_DIRECTION'].$titanium_lang_donate['PREV'].'</a></strong>';
	    } else {
	    ...
	    if ($stop != sizeof($donations)) {
	        $next_page = $page + 1;
	        $next = '<strong><a href="modules.php?name=Donations&op=view&page='.$next_page.'">'.$titanium_lang_donate['NEXT'].$titanium_lang_donate['NEXT_DIRECTION'].'</a></strong>';
	    } else {
...
function view_display_links () {
    global $titanium_lang_donate, $gen_configs;

    OpenTable();
    echo '<table width="35%" border="1" style="margin: auto">';
    echo '  <tr>';
    echo '    <td style="width: 50%"><span class="tooltip-html" title="'.$titanium_lang_donate['HELP_TOTAL'].'"><a href="modules.php?name=Donations&amp;op=view&amp;dis=total">'.$titanium_lang_donate['TOTAL'].'</a></span></td>';
    echo '    <td style="width: 50%"><span class="tooltip-html" title="'.$titanium_lang_donate['HELP_GOAL'].'"><a href="modules.php?name=Donations&amp;op=view&amp;dis=goal">'.$titanium_lang_donate['GOAL'].'</a></span></td>';
    echo '  </tr>';
    echo '</table>';
