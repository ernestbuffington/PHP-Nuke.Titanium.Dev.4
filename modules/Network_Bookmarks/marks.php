...
{
	$getname="select name from ".$network_prefix."_bookmarks_cat where category_id='$category'";
...
$marks_query = "select id,name,url,description,mod_date,popup from " . $network_prefix . "_bookmarks where user_id=" . $userid . " and category_id='" . $category . "' order by name";
...
for ($i=0;$i<@$db->sql_numrows  ($marks_res,$db);$i++)
{
    ...
    global $db;
    list($fixed_markurl) = $db->sql_ufetchrow("SELECT `url` FROM `".$network_prefix."_bookmarks` WHERE `id`='".$marks_row['id']."'", SQL_NUM);
