...
if(!empty($welcome_pm) && !empty($submit)) {
    ...
    if($titanium_db->sql_numrows($titanium_db->sql_query("SELECT * FROM ".$titanium_prefix."_welcome_pm")) != 0) {
        $sql_w_pm = "UPDATE ".$titanium_prefix."_welcome_pm SET subject='".$HTTP_POST_VARS['subject']."', msg='".$HTTP_POST_VARS['message']."'";
    } else {
        $sql_w_pm = "INSERT INTO ".$titanium_prefix."_welcome_pm VALUES('".$HTTP_POST_VARS['subject']."', '".$HTTP_POST_VARS['message']."')";
    }
...
$phpbb2_start = ( !empty($HTTP_GET_VARS['start']) ) ? intval($HTTP_GET_VARS['start']) : 0;
...
else if ( $submit || $refresh || !empty($mode) )
{
        if ( !$userdata['session_logged_in'] )
        {
                $titanium_user_id = ( isset($HTTP_GET_VARS[POST_USERS_URL]) ) ? '&' . POST_USERS_URL . '=' . intval($HTTP_GET_VARS[POST_USERS_URL]) : '';
                // not needed anymore due to function redirect_titanium()
//$header_location = ( @preg_match('/Microsoft|WebSTAR|Xitami/', $_SERVER['SERVER_SOFTWARE']) ) ? 'Refresh: 0; URL=' : 'Location: ';
 ...
 ******************************************************/
        ...
        else
        {
                ...
                {
                        $titanium_user_id = intval($HTTP_GET_VARS[POST_USERS_URL]);
