...
// Language File
//
include($phpbb2_root_path.'language/lang_'.$phpbb2_board_config['default_lang'].'/lang_admin_rebuild_search.'.$phpEx);
...
{
  if ( $phpbb2_board_config['board_disable'] )
  {
    $disable_board = 2;
  }
  else
...
if ( isset($HTTP_GET_VARS['cancel_button']) || isset($HTTP_POST_VARS['cancel_button'])  )
{
  ...
  $phpbb2_end_post_id = get_rebuild_session_details('last', 'end_post_id');
...
{
  $phpbb2_start = ( isset($HTTP_POST_VARS['start']) ) ? intval($HTTP_POST_VARS['start']) : intval($HTTP_GET_VARS['start']);
}
else
{
  $phpbb2_start = -1;
...
$phpbb2_total_posts = get_total_posts();

// get the number of total/session posts already processed
$phpbb2_total_posts_processed = ( $phpbb2_start != 0 ) ? get_total_posts('before', get_rebuild_session_details('last', 'end_post_id')) : 0;
$session_posts_processed = ( $mode == 'refresh' ) ? get_processed_posts('session') : 0;

// find how many posts aren't processed
$phpbb2_total_posts_processing = $phpbb2_total_posts - $phpbb2_total_posts_processed;
...
if ( isset($HTTP_GET_VARS['session_posts_processing']) || isset($HTTP_POST_VARS['session_posts_processing']) )
{
  ...
  if ( $mode == 'submit' )
  {
    ...
    if ( $session_posts_processing + $phpbb2_total_posts_processed > $phpbb2_total_posts )
    {
      $session_posts_processing = $phpbb2_total_posts - $phpbb2_total_posts_processed;
    }
  }

  // correct it when posts are deleted during processing
  $session_posts_processing = ( $session_posts_processing > $phpbb2_total_posts) ? $phpbb2_total_posts : $session_posts_processing;
}
else
{
  // if we have finished, get all the posts, else only the remaining
  $session_posts_processing = ( $phpbb2_total_posts_processing == 0 ) ? $phpbb2_total_posts : $phpbb2_total_posts_processing;
}
...
if ( $mode == 'submit' || $mode == 'refresh' )
{
  ...
  $phpbb2_start_post_id = 0;
  $phpbb2_end_post_id = 0;
  ...
  while ( ($row = $titanium_db->sql_fetchrow($result)) && ($timer_expired == false) )
  {
    ...
    if ( $num_rows == 0 )
    {
      $phpbb2_start_post_id = $row['post_id'];
    }
    $phpbb2_end_post_id = $row['post_id'];

    // calculate time for each loop iteration
    $temp_time[$num_rows] = time() - $phpbb2_start_temp_time;
    // get the max
  ...
  if ( $num_rows != 0 )
  {
    if ( $mode == 'submit' )
    {
      ...
      $sql = "INSERT INTO " . SEARCH_REBUILD_TABLE . "
        ...
        VALUES ($phpbb2_start_post_id, $phpbb2_end_post_id, $cycle_start_time, $cycle_end_time, $last_cycle_time, $last_cycle_time, $num_rows, 1, " . intval($search_tables_size). ", " . REBUILD_SEARCH_PROCESSED . ")";
    }
    else  // refresh
    {
      ...
      $sql = "UPDATE " . SEARCH_REBUILD_TABLE . "
        SET
          end_post_id = $phpbb2_end_post_id,
  ...
  $phpbb2_total_posts_processed = get_total_posts('before', get_rebuild_session_details('last','end_post_id'));
  $phpbb2_total_posts = get_total_posts();

  // check if there is more data to be processed
  if ( $session_posts_processed < $session_posts_processing && $phpbb2_total_posts_processed < $phpbb2_total_posts )
  {
    $form_parameters = '&start='.($phpbb2_end_post_id + 1);
...
else  // show the input page
{
  ...
  $post_limit_max = ( $def_post_limit > $phpbb2_total_posts ) ? $phpbb2_total_posts : $def_post_limit;

  //$s_hidden_fields = '<input type="hidden" name="post_limit_stored" value="'.$post_limit_max.'" />';
  //$s_hidden_fields .= '<input type="hidden" name="total_posts_stored" value="'.$phpbb2_total_posts.'" />';
  ...
  if ( !empty($last_session_details) )
  {
    ...
    $last_saved_date = create_date($phpbb2_board_config['default_dateformat'], $last_session_details['end_time'], $phpbb2_board_config['board_timezone']);
    ...
    $phpbb2_template->assign_block_vars("last_saved_info", array());
  }
  else
  {
    $phpbb2_template->assign_block_vars("start_text_input", array());
  ...
  $phpbb2_template->assign_vars(array(
    ...
    'S_REBUILD_SEARCH_ACTION' => append_titanium_sid("admin_rebuild_search.$phpEx?mode=submit" . (( $disable_board == 2 ) ? "&disabled_board_status=$disable_board" : ""))
    )
  );
}
