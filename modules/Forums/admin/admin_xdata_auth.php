	...
	if ( ( $mode == 'edit' || $mode == 'save' ) && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
	{
		...
		else
		{
			...
			*/
		    ...
		    $message = sprintf($lang['XData_success_updating_permissions'],"<a href=\"" . append_titanium_sid("admin_xdata_auth.$phpEx?type=user") . "\">","</a>");
			$message .= sprintf($lang['Click_return_admin_index'], "<a href=\"" . append_titanium_sid("index.$phpEx?pane=right") . "\">", "</a>");
