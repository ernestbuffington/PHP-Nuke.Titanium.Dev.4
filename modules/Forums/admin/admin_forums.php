 ...
 ******************************************************/
                        ...
                        $sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, 
                                ...
                                VALUES ('" . $next_id . "', '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " . intval($HTTP_POST_VARS['forum_is_link']) . ", '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_weblink']) . "', '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_link_icon']) . "', " . intval($HTTP_POST_VARS['forum_link_target']) . ", '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_color']) . "', " . $new_cat . ', ' . $new_parent . ", '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['password']) . "', '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', " . intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ", " 
 ...
 ******************************************************/
                        $sql = "UPDATE " . FORUMS_TABLE . "
                                ...
                                SET forum_name = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', title_is_link = " 
								. intval($HTTP_POST_VARS['forum_is_link']) . ", weblink = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_weblink']) . "', forum_link_icon = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_link_icon']) . "', forum_link_target = " 
								. intval($HTTP_POST_VARS['forum_link_target']) . ", forum_color = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forum_color']) . "', cat_id = $new_cat, forum_parent = $new_parent, forum_desc = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', forum_status = " 
								. intval($HTTP_POST_VARS['forumstatus']) . ", forum_password = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['password']) . "', forum_icon = '" 
								. str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', forum_display_order = " 
								. intval($HTTP_POST_VARS['forum_display_order']) . ", forum_display_sort = " 
								. intval($HTTP_POST_VARS['forum_display_sort']) . ", prune_enable = " 
								. intval($HTTP_POST_VARS['prune_enable']) . ", forum_thank = " 
								. intval($HTTP_POST_VARS['forumthank']) . "
 ...
 ******************************************************/
						if( !$forum_info['forum_parent'] )
						{
							...
							if( $move > 0 )
							{
								...
								AND forum_order > '" . $forum_info['forum_order'] . "' 
 ...
 ******************************************************/ 
										...
										'NUM_TOPICS' => ($forum_rows[$j]['forum_link_icon'] && $forum_rows[$j]['title_is_link']) 
										? ((substr($forum_rows[$j]['forum_link_icon'],0,4) == 'http' 
										|| substr($forum_rows[$j]['forum_link_icon'],0,4) == 'HTTP' ) 
										? '<img src="'.$forum_rows[$j]['forum_link_icon'].'" border="0">' : '<img 
										src="./../' . $forum_rows[$j]['forum_link_icon'] . '" border="0">') : $forum_rows[$j]['forum_topics'],

										'NUM_POSTS' => ($forum_rows[$j]['forum_link_icon'] && $forum_rows[$j]['title_is_link']) ? '' : $forum_rows[$j]['forum_posts'],

										'U_VIEWFORUM' => ($forum_rows[$j]['title_is_link']) 
										? $forum_rows[$j]['weblink'] : ("../../../modules.php?name=Forums&file=viewforum&amp;" . POST_FORUM_URL . "=$phpbb2_forum_id"),
/*****[END]********************************************
 [ Mod:    Forumtitle as Weblink               v1.2.2 ]
 ******************************************************/ 
