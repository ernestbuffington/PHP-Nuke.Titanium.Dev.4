...
$arcade_catid = get_var_gf(['name' => 'cid', 'intval' => true]);

$start = get_var_gf(['name' => 'start', 'intval' => true]);
...
$order_by = match ($arcade_config['game_order']) {
    'Alpha' => ' game_name ASC ',
    'Popular' => ' game_set DESC ',
    'Fixed' => ' game_order ASC ',
    'Random' => ' RAND() ',
    'News' => ' game_id DESC ',
    default => ' game_order ASC ',
};
...
if(($arcade_catid == 0 ) && ($arcade_config['use_category_mod'])):
  ...
  $template->set_filenames(['body' => 'arcade_cat_body.tpl']);



  $template->assign_vars(['URL_ARCADE' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "arcade.$phpEx" ) . '">' . $lang[ 'lib_arcade' ] . '</a></nobr> ', 'URL_BESTSCORES' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "toparcade.$phpEx" ) . '">' . $lang[ 'best_scores' ] . '</a></nobr> ', 'URL_SCOREBOARD' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "scoreboard.$phpEx?gid=$gid" ) . '">' . $lang[ 'scoreboard' ] . '</a></nobr> ', 'MANAGE_COMMENTS' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "comments_list.$phpEx" ) . '">' . $lang[ 'comments' ] . '</a></nobr> ', 'ARCADE_COL' => ( $arcade_config[ 'use_fav_category' ] ) ? 6 : 5, 'ARCADE_COL1' => ( $arcade_config[ 'use_fav_category' ] ) ? 2 : 1, 'FAV' => $lang[ 'fav' ], 'L_GAME' => $lang[ 'games' ], 'L_HIGHSCORE' => $lang[ 'highscore' ], 'L_YOURSCORE' => $lang[ 'yourbestscore' ], 'L_DESC' => $lang[ 'desc_game' ], 'L_ARCADE' => $lang[ 'lib_arcade' ]] );
  ...
  if($arcade_config['use_fav_category' ]):
    ...
    if($db->sql_numrows($result)):
      ...
      $template->assign_block_vars('favrow',[]);
       ...
       while($frow = $db->sql_fetchrow($result)) :
		...
		$template->assign_block_vars('favrow.fav_row',[
      ...
      'GAMEDESCF' => '<span class="arcadeTextDescription">'.$frow[ 'game_desc' ].'</span>',
           ...
           if($frow['game_highscore'] != 0 ): 
             ...
             $template->assign_block_vars( 'favrow.fav_row.recordrow', [] );
           ...
           if($frow['score_game'] != 0 ):
             ...
             $template->assign_block_vars( 'favrow.fav_row.yourrecordrow', [] );

           else: 

           $template->assign_block_vars( 'favrow.fav_row.playrecordrow', [] );
  ...
  while($row = $db->sql_fetchrow($result)):
    if (!empty($nbjeux) &&(sizeof($liste_jeux[ $row[ 'arcade_catid' ] ]) < 1)) # added for PHP 8
    ...
    if($nbjeux > 0 ):
      ...
      $template->assign_block_vars( 'cat_row', ['U_ARCADE' => append_sid( "arcade.$phpEx?cid=" . $row[ 'arcade_catid' ] ), 'LINKCAT_ALIGN' => ( $arcade_config[ 'linkcat_align' ] == '0' ) ? 'left' : ( ( $arcade_config[ 'linkcat_align' ] == '1' ) ? 'center' : 'right' ), 'L_ARCADE' => sprintf( $lang[ 'Other_games' ], $row[ 'arcade_nbelmt' ] ), 'CATTITLE' => $row[ 'arcade_cattitle' ]] );
      ...
      for($i = 0; $i < $nbjeux; $i++ ):
        ...
        $template->assign_block_vars( 'cat_row.game_row', ['GAMENAME' => $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_name' ], 'GAMELINK' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "games.$phpEx?gid=" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . '">' . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_name' ] . '</a></nobr> ', 'GAMEPOPUPLINK' => "<a class='popup' href='javascript:Arcade_Popup(\"" . append_sid( "gamespopup.$phpEx?gid=" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . "\", \"New_Window\",\"" 

    		  . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_width' ] . "\",\"" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_height' ] . "\", \"no\")'>New Window</a>", 'GAMEPIC' => ( $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_pic' ] != '' ) ? "<a class='rounded-corners-arcade' width='60' href='" . append_sid( "games.$phpEx?gid=" 

    		  . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . "'><img class='rounded-corners-arcade' width='60' src='" . $phpbb_root_path . "games/pics/" 

    		  . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_pic' ] . "' align='absmiddle' border='0' vspace='2' hspace='2' alt='" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_name' ] . "' ></a>" : '', 'GAMESET' => ( $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_set' ] != 0 ) ? '<span class="arcadeTextWhite">'.$lang[ 'game_actual_nbset' ].'</span>' 

    		  .'<span class="w3-badge w3-blue"><strong>'.$liste_jeux[ $row['arcade_catid']][$i]['game_set'].'</strong></span>' : '', 'HIGHSCORE' => '<span class="genmed w3-tag w3-round w3-green w3-border w3-border-pink">'.$liste_jeux[$row['arcade_catid' ]][$i]['game_highscore'].'</span>' , 'YOURHIGHSCORE' => '<span class="genmed w3-tag w3-round w3-green w3-border w3-border-pink">'.$liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'score_game' ].'</span>' , 'CLICKPLAY' => '<a class="clicktoplay" href="' . append_sid( "games.$phpEx?gid=" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . '">Click to Play!</a>', 'NORECORD' => ( $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_highscore' ] == 0 ) ? $lang[ 'no_record' ] : '', 'HIGHUSER' => ($liste_jeux[$row['arcade_catid']][$i]['game_highuser'] != 0) ? '' .'<span class="arcadeTextWhite">'.$liste_jeux[$row['arcade_catid']][$i]['username'].'</span>'.'' : '', 'URL_SCOREBOARD' => '<nobr><a class="cattitle" href="' . append_sid( "scoreboard.$phpEx?gid=" . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . '">' . "<img 

		  src='" . $phpbb_root_path . "templates/" . $theme[ 'template_name' ] . "/images/scoreboard.gif' align='absmiddle' border='0' alt='" . $lang[ 'scoreboard' ] . " " 

    		  . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_name' ] . "'>" . '</a></nobr> ', 'GAMEID' => $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ], 'DATEHIGH' => "<nobr><span class='arcadeTextDate'>".create_date($board_config['default_dateformat'],$liste_jeux[$row['arcade_catid']][$i]['game_highdate'],$board_config['board_timezone'])."</span></nobr>", 'YOURDATEHIGH' => "<nobr><span class='arcadeTextDate'>" . create_date( $board_config[ 'default_dateformat' ], $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'score_date' ], $board_config[ 'board_timezone' ] ) 

    		  . "</span></nobr>", 'IMGFIRST' => ( $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_highuser' ] == $userinfo[ 'user_id' ] ) ? "<span class='arcadeTextDate'>1st Place</span>".$br."<img 

		  src='" . $phpbb_root_path . "templates/" . $theme[ 'template_name' ] . "/images/couronne.gif' align='absmiddle'>" : "", 'ADD_FAV' => ( $arcade_config[ 'use_fav_category' ] ) ? '<td class="arcadeRow1" width="8%" align="center" valign="center"><a class="arcadeTitleLink" href="' . append_sid( "arcade.$phpEx?favori=" 

    		  . $liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_id' ] ) . '"><i class="bi bi-plus-square"></i></br><font size="2">Add Favorite</font></a></td>' : '', 'GAMEDESC' => '<span class="arcadeTextDescription">'.$liste_jeux[ $row[ 'arcade_catid' ] ][ $i ][ 'game_desc' ].'</span>'] );



        if($liste_jeux[$row['arcade_catid']][$i]['game_highscore'] != 0): 

          $template->assign_block_vars( 'cat_row.game_row.recordrow', [] );
        ...
        if($liste_jeux[$row['arcade_catid']][$i]['score_game'] != 0):
          ...
          $template->assign_block_vars( 'cat_row.game_row.yourrecordrow', [] );

        else: 

          $template->assign_block_vars( 'cat_row.game_row.playrecordrow', [] );
...
if($arcade_config['use_category_mod']):
  ...
  $template->assign_block_vars('use_category_mod',[] );
...
$template->set_filenames( ['body' => 'arcade_body.tpl'] );



$template->assign_vars( ['URL_ARCADE' => '<nobr><a class="arcadeTitleLink" href="' . append_sid( "arcade.$phpEx" ) . '">' . $lang[ 'lib_arcade' ] . '</a></nobr> ', 'URL_BESTSCORES' => '<nobr><a class="cattitle" href="' . append_sid( "toparcade.$phpEx" ) . '">' . $lang[ 'best_scores' ] . '</a></nobr> ', 'URL_SCOREBOARD' => '<nobr><a class="cattitle" href="' . append_sid( "scoreboard.$phpEx?gid=$gid" ) . '">' . $lang[ 'scoreboard' ] . '</a></nobr> ', 'MANAGE_COMMENTS' => '<nobr><a class="cattitle" href="' . append_sid( "comments_list.$phpEx" ) . '">' . $lang[ 'comments' ] . '</a></nobr> ', 'CATTITLE' => $row[ 'arcade_cattitle' ], 'NAV_DESC' => '<a class="nav" href="' . append_sid( "arcade.$phpEx" ) . '">' . $lang[ 'arcade' ] . '</a> ', 'L_GAME' => $lang[ 'games' ], 'PAGINATION' => generate_pagination( append_sid( "arcade.$phpEx?cid=$arcade_catid" ), $total_games, $games_par_page, $start ), 'PAGE_NUMBER' => sprintf( $lang[ 'Page_of' ], ( floor( $start / $games_par_page ) + 1 ), ceil( $total_games / $games_par_page ) ), 'ARCADE_COL' => ( $arcade_config[ 'use_fav_category' ] ) ? 6 : 5, 'ARCADE_COL1' => ( $arcade_config[ 'use_fav_category' ] ) ? 2 : 1, 'FAV' => $lang[ 'fav' ], 'L_HIGHSCORE' => $lang[ 'highscore' ], 'L_YOURSCORE' => $lang[ 'yourbestscore' ], 'L_DESC' => $lang[ 'desc_game' ], 'L_ARCADE' => $lang[ 'lib_arcade' ]] );
...
while($row = $db->sql_fetchrow($result)):
  ...
  $template->assign_block_vars('gamerow',[
  ...
  if($row['game_highscore'] != 0):
    ...
    $template->assign_block_vars( 'gamerow.recordrow', [] );
  ...
  if($row['score_game' ] != 0):
    ...
    $template->assign_block_vars( 'gamerow.yourrecordrow', [] );

  else:

    $template->assign_block_vars( 'gamerow.playrecordrow', [] );
