...
if (is_admin())
{
    ...
    while ( $row = $pnt_db->sql_fetchrow($result) )
    {
       ...
       if ( ($result1 = $pnt_db->sql_query($sql1)) )
       {
           $row1 = $pnt_db->sql_fetchrow($result1);
              $pnt_usergroup =( ( $row1['group_id'] != '' ) ? $row1['group_id'] : 'User has no user group'.$row1 );
             ...
             if ( !($row2 = $pnt_db->sql_fetchrow($result2)) )
             {
             ...
             if ( !($result3 = $pnt_db->sql_query($sql3)) )
             {
             ...
             if( !($result4 = $pnt_db->sql_query($sql4)) )
             { 
                message_die(GENERAL_ERROR, 'Could not insert data into user_group table', '', __LINE__, __FILE__, $sql4);
             }

                  
                 $pnt_usergroup = $pnt_usergroup.', adding user group '.$group_id;
              }


       $liste .= ( ( $liste != '' ) ? '<br /> ' : '' ) . $pnt_username.' <b>'.$pnt_usergroup.'</b>';
    }
