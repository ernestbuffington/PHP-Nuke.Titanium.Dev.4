...
/*======================================================================= 
 ...
 =======================================================================*/
//http://127.0.0.1/?act=Arcade&do=newscore
...
if($mode == "submit")
{
    ...
    $sql = "SELECT s.score_set, s.game_id, g.game_name 
	FROM " . SCORES_TABLE. " s LEFT JOIN " . USERS_TABLE. " u ON s.user_id = u.user_id LEFT JOIN " . GAMES_TABLE. " g ON s.game_id = g.game_id WHERE s.user_id = " . $userdata['user_id'] . " ORDER BY score_set DESC LIMIT 1";
	...
	# Gets the number of highscores for the current user
