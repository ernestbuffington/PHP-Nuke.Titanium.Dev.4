...
function changemail() {
    ...
    list($get_username, $tuemail) = $titanium_db -> sql_fetchrow($titanium_db -> sql_query("SELECT username, user_email FROM ".$titanium_user_prefix."_users WHERE user_id = '$get_id'"));
    $datekey = date("F Y");
    ...
    if ((is_user()) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo[user_password] == $cookie[2])) {
        if ($stop == '') {
            if ( (strtolower($userinfo['username']) == strtolower($get_username)) AND ($check_num2 == $check_num) ) {
                $result = $titanium_db->sql_query("UPDATE ".$titanium_user_prefix."_users SET user_email='$newmail' WHERE user_id='$get_id'");
