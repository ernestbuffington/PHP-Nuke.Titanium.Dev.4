...
while(list($project_id) = $titanium_db2->sql_fetchrow($projectresult)) {
  ...
  if($project['allowreports'] > 0) {
    $reportresult = $titanium_db2->sql_query("SELECT `report_id`, `report_name`, `status_id`, `type_id` FROM `".$network_prefix."_reports` WHERE `project_id`='$project_id' ORDER BY `report_name`");
    ...
    if($report_total > 0){
      list($last_date) = $titanium_db2->sql_fetchrow($titanium_db2->sql_query("SELECT `date_submitted` FROM `".$network_prefix."_reports` WHERE `project_id`='$project_id' ORDER BY `date_submitted` desc"));
    ...
    if($report_total != 0){
      while(list($report_id, $report_name, $status_id, $type_id) = $titanium_db2->sql_fetchrow($reportresult)) {
        ...
        if($reportcomment_total > 0){
          list($last_date) = $titanium_db2->sql_fetchrow($titanium_db2->sql_query("SELECT `date_commented` FROM `".$network_prefix."_reports_comments` WHERE `report_id`='$report_id' ORDER BY `date_commented` desc"));
          $last_date = date($pj_config['report_date_format'], $last_date);    
        } else {
          $last_date = _NETWORK_NA;
        }
