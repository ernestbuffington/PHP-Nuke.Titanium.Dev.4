...
$totalselected = $titanium_db->sql_numrows($titanium_db->sql_query("SELECT `rid` FROM `".$titanium_prefix."_nsnst_referers`"));
if($totalselected > 0) {
  ...
  echo '</tr>'."\n";
  $result = $titanium_db->sql_query("SELECT * FROM `".$titanium_prefix."_nsnst_referers` ORDER BY `referer` $direction LIMIT $min,$perpage");
