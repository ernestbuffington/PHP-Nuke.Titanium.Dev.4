...
abstract class Smarty_Internal_TemplateCompilerBase
{
     ...
     *
     * @param  Smarty_Internal_Template $phpbb2_template template object to compile
     * @param  bool $nocache    true is shall be compiled in nocache mode
     * @return bool             true if compiling succeeded, false if it failed
     */
    public function compileTemplate(Smarty_Internal_Template $phpbb2_template, $nocache = false)
    {
        if (empty($phpbb2_template->properties['nocache_hash'])) {
            $phpbb2_template->properties['nocache_hash'] = $this->nocache_hash;
        } else {
            $this->nocache_hash = $phpbb2_template->properties['nocache_hash'];
        }
        // flag for nochache sections
        $this->nocache = $nocache;
        $this->tag_nocache = false;
        // save template object in compiler class
        $this->template = $phpbb2_template;
        // reset has nocache code flag
        $this->template->has_nocache_code = false;
        $save_source = $this->template->source;
        // template header code
        $phpbb2_template_header = '';
        if (!$this->suppressHeader) {
            $phpbb2_template_header .= "<?php /* Smarty version " . Smarty::SMARTY_VERSION . ", created on " . strftime("%Y-%m-%d %H:%M:%S") . "\n";
            $phpbb2_template_header .= "         compiled from \"" . $this->template->source->filepath . "\" */ ?>\n";
        }

        if (empty($this->template->source->components)) {
            $this->sources = array($phpbb2_template->source);
        } else {
            // we have array of inheritance templates by extends: resource
            $this->sources = array_reverse($phpbb2_template->source->components);
        }
        $loop = 0;
            ...
            do {
                ...
                if ($_content != '') {
                    ...
                    if ((isset($this->smarty->autoload_filters['pre']) || isset($this->smarty->registered_filters['pre'])) && !$this->suppressFilter) {
                        $_content = Smarty_Internal_Filter_Handler::runFilter('pre', $_content, $phpbb2_template);
                    }
                    // call compiler
        ...
        if ((isset($this->smarty->autoload_filters['post']) || isset($this->smarty->registered_filters['post'])) && !$this->suppressFilter && $_compiled_code != '') {
            $_compiled_code = Smarty_Internal_Filter_Handler::runFilter('post', $_compiled_code, $phpbb2_template);
        }
        if ($this->suppressTemplatePropertyHeader) {
            $code = $_compiled_code . $merged_code;
        } else {
            $code = $phpbb2_template_header . $phpbb2_template->createTemplateCodeFrame($_compiled_code) . $merged_code;
        }
        // unset content because template inheritance could have replace source with parent code
        unset ($phpbb2_template->source->content);
    ...
    public function getPluginFromDefaultHandler($tag, $plugin_type)
    {
        $callback = null;
        $script = null;
        $titanium_cacheable = true;
        $result = call_user_func_array(
            $this->smarty->default_plugin_handler_func, array($tag, $plugin_type, $this->template, &$callback, &$script, &$titanium_cacheable)
        );
        if ($result) {
            $this->tag_nocache = $this->tag_nocache || !$titanium_cacheable;
            if ($script !== null) {
                if (is_file($script)) {
                    if ($this->template->caching && ($this->nocache || $this->tag_nocache)) {
