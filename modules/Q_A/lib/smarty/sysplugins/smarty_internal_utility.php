...
class Smarty_Internal_Utility
{
     ...
     */
    public static function getTags(Smarty_Internal_Template $phpbb2_template)
    {
        $phpbb2_template->smarty->get_used_tags = true;
        $phpbb2_template->compileTemplateSource();

        return $phpbb2_template->used_tags;
    }
    ...
    public static function testInstall(Smarty $smarty, &$errors=null)
    {
        ...
        foreach ($smarty->getTemplateDir() as $phpbb2_template_dir) {
            $_template_dir = $phpbb2_template_dir;
            $phpbb2_template_dir = realpath($phpbb2_template_dir);
            // resolve include_path or fail existence
            if (!$phpbb2_template_dir) {
                if ($smarty->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $_template_dir)) {
                    // try PHP include_path
                    if ($_stream_resolve_include_path) {
                        $phpbb2_template_dir = stream_resolve_include_path($_template_dir);
                    } else {
                        $phpbb2_template_dir = Smarty_Internal_Get_Include_Path::getIncludePath($_template_dir);
                    }

                    if ($phpbb2_template_dir !== false) {
                        if ($errors === null) {
                            echo "$phpbb2_template_dir is OK.\n";
                        }
            ...
            if (!is_dir($phpbb2_template_dir)) {
                $status = false;
                $message = "FAILED: $phpbb2_template_dir is not a directory";
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
                    $errors['template_dir'] = $message;
                }
            } elseif (!is_readable($phpbb2_template_dir)) {
                $status = false;
                $message = "FAILED: $phpbb2_template_dir is not readable";
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
                    $errors['template_dir'] = $message;
