 ...
 * @return string
 */
function smarty_function_html_select_date($params, $phpbb2_template)
{
    ...
    // make syntax "+N" or "-N" work with $phpbb2_start_year and $phpbb2_end_year
    // Note preg_match('!^(\+|\-)\s*(\d+)$!', $phpbb2_end_year, $match) is slower than trim+substr
    ...
    if (($phpbb2_start_year > $phpbb2_end_year && !$reverse_years) || ($phpbb2_start_year < $phpbb2_end_year && $reverse_years)) {
        $t = $phpbb2_end_year;
        $phpbb2_end_year = $phpbb2_start_year;
        $phpbb2_start_year = $t;
    }
    ...
    if ($display_years) {
        ...
        if ($year_as_text) {
            ...
            $op = $phpbb2_start_year > $phpbb2_end_year ? -1 : 1;
            for ($i=$phpbb2_start_year; $op > 0 ? $i <= $phpbb2_end_year : $i >= $phpbb2_end_year; $i += $op) {
