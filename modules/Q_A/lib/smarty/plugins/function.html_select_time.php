...
function smarty_function_html_select_time($params, $phpbb2_template)
{
    ...
    if (isset($params['time']) && is_array($params['time'])) {
        if (isset($params['time'][$titanium_prefix . 'Hour'])) {
            ...
            $_meridian = isset($params['time'][$titanium_prefix . 'Meridian'])
                ? (' ' . $params['time'][$titanium_prefix . 'Meridian'])
                : '';
            $time = strtotime( $_hour . ':' . $_minute . ':' . $_second . $_meridian );
            list($_hour, $_minute, $_second) = $time = explode('-', date('H-i-s', $time));
        } elseif (isset($params['time'][$field_array][$titanium_prefix . 'Hour'])) {
            // $_REQUEST given
            foreach (array('H' => 'Hour',  'i' => 'Minute', 's' => 'Second') as $_elementKey => $_elementName) {
                $_variableName = '_' . strtolower($_elementName);
            ...
            $_meridian = isset($params['time'][$field_array][$titanium_prefix . 'Meridian'])
                ? (' ' . $params['time'][$field_array][$titanium_prefix . 'Meridian'])
                : '';
            $time = strtotime( $_hour . ':' . $_minute . ':' . $_second . $_meridian );
            list($_hour, $_minute, $_second) = $time = explode('-', date('H-i-s', $time));
        } else {
    ...
    if ($display_hours) {
        $_html_hours = '';
        $_extra = '';
        $_name = $field_array ? ($field_array . '[' . $titanium_prefix . 'Hour]') : ($titanium_prefix . 'Hour');
        if ($all_extra) {
            $_extra .= ' ' . $all_extra;
        }
        if ($hour_extra) {
        ...
        $phpbb2_start = $use_24_hours ? 0 : 1;
        $phpbb2_end = $use_24_hours ? 23 : 12;
    ...
    if ($display_minutes) {
        $_html_minutes = '';
        $_extra = '';
        $_name = $field_array ? ($field_array . '[' . $titanium_prefix . 'Minute]') : ($titanium_prefix . 'Minute');
        if ($all_extra) {
            $_extra .= ' ' . $all_extra;
        }
        if ($minute_extra) {
    ...
    if ($display_seconds) {
        $_html_seconds = '';
        $_extra = '';
        $_name = $field_array ? ($field_array . '[' . $titanium_prefix . 'Second]') : ($titanium_prefix . 'Second');
        if ($all_extra) {
            $_extra .= ' ' . $all_extra;
        }
        if ($second_extra) {
    ...
    if ($display_meridian && !$use_24_hours) {
        $_html_meridian = '';
        $_extra = '';
        $_name = $field_array ? ($field_array . '[' . $titanium_prefix . 'Meridian]') : ($titanium_prefix . 'Meridian');
        if ($all_extra) {
            $_extra .= ' ' . $all_extra;
        }
        if ($meridian_extra) {
