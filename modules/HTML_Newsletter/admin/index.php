...
$sql 						= "SELECT `name`, `radminsuper` FROM `".$pnt_prefix."_authors` WHERE `aid`='$aid'";
...
if ( $row['radminsuper'] == 1 ) {  //No need to go any further - we have a super admin
	...
	if ( defined( 'MSNL_PRE75_LOADED' ) ) { //Need to do this the "old" way
		...
		$sql 						= "SELECT `name`, `radminsuper`, `radminnewsletter` FROM `".$pnt_prefix."_authors` WHERE `aid`='$aid'";
		...
		$sql						= "SELECT `title`, `admins` FROM `".$pnt_prefix."_modules` WHERE `title`='$msnl_sModuleNm'";
		$result1 				= msnl_fSQLCall( $sql );
		$row1						= $pnt_db->sql_fetchrow( $result1 );
