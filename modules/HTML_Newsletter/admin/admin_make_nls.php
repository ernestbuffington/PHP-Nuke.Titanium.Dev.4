...
if ( $_POST['msnl_stats'] == "yes" && $msnl_sTemplateNm != "notemplate" ) {
	...
	$sql									= "SELECT `user_id` FROM `".$pnt_prefix."_users` WHERE `username` <> 'Anonymous'";
	...
	if ( !$result ) { //Bad SQL call
		...
		$msnl_iStatsTotUsr		= intval( $pnt_db->sql_numrows( $result ) );
	
	}

	//Total Hits
	
	$sql										= "SELECT `count` FROM `".$pnt_prefix."_counter` WHERE `type` = 'total' "
	...
	$sql										= "SELECT * FROM `".$pnt_prefix."_stories`";
	...
	if ( !$result2 ) { //Bad SQL call
		...
		$msnl_iStatsTotNews			= intval( $pnt_db->sql_numrows( $result2 ) );

	}

	//Total News categories
	
	$sql										= "SELECT * FROM `".$pnt_prefix."_stories_cat`";
	...
	if ( !$result3 ) { //Bad SQL call
		...
		$msnl_iStatsTotNewsCat	= intval( $pnt_db->sql_numrows( $result3 ) );

	}

	//Total Downloads Files

	$sql										= "SELECT * FROM `".$pnt_prefix."_".$msnl_gasModCfg['dl_module']."_downloads`";
	...
	if ( !$result4 ) { //Bad SQL call
		...
		$msnl_iStatsTotDls			= intval( $pnt_db->sql_numrows( $result4 ) );

	}
	
	//Total Downloads Categories

	$sql										= "SELECT * FROM `".$pnt_prefix."_".$msnl_gasModCfg['dl_module']."_categories`";
	...
	if ( !$result5 ) { //Bad SQL call
		...
		$msnl_iStatsTotDlsCat		= intval( $pnt_db->sql_numrows( $result5 ) );

	}
	
	//Total Web Links

	$sql										= "SELECT * FROM `".$pnt_prefix."_links_links`";
	...
	if ( !$result6 ) { //Bad SQL call
		...
		$msnl_iStatsTotLnks			= intval( $pnt_db->sql_numrows( $result6 ) );
	
	}
	
	//Total Web Links Categories

	$sql										= "SELECT * FROM `".$pnt_prefix."_links_categories`";
	...
	if ( !$result7 ) { //Bad SQL call
		...
		$msnl_iStatsTotLnksCat	= intval( $pnt_db->sql_numrows( $result7 ) );

	}
	
	//Total Amount of Forum Topics

	$sql										= "SELECT * FROM `".$pnt_prefix."_bbtopics`";
	...
	if ( !$result8 ) { //Bad SQL call
		...
		$msnl_iStatsTotForums		= intval( $pnt_db->sql_numrows( $result8 ) );
	
	}
	
	//Total Amount of forum Posts

	$sql										= "SELECT * FROM `".$pnt_prefix."_bbposts`";
	...
	if ( !$result9 ) { //Bad SQL call
		...
		$msnl_iStatsTotPosts		= intval( $pnt_db->sql_numrows( $result9 ) );
	
	}
	
	//Total Amount of Reviews

	$sql										= "SELECT * FROM `".$pnt_prefix."_reviews`";
	...
	if ( !$result10 ) { //Bad SQL call
		...
		$msnl_iStatsTotReviews	= intval( $pnt_db->sql_numrows( $result10 ) );
...
if ( $_POST['msnl_news'] > 0 && $msnl_sTemplateNm != "notemplate" ) {
	...
	$sql				= "SELECT `sid`, `informant`, `title`, `topic`, `topictext`, a.`counter` AS counter FROM `"
							. $pnt_prefix."_stories` a, `"
							. $pnt_prefix."_topics` b "
	...
	if ( !$result11 ) { //Bad SQL call
		...
		while ( $row = $pnt_db->sql_fetchrow( $result11 ) ) {
...
if ( $_POST['msnl_downloads'] > 0 && $msnl_sTemplateNm != "notemplate" ) {
	...
	$sql				= "SELECT `lid`, a.`cid` as cid, a.`title` as dtitle, `hits`, b.`title` as ctitle FROM `"
							. $pnt_prefix."_".$msnl_gasModCfg['dl_module']."_downloads` a, `"
							. $pnt_prefix."_".$msnl_gasModCfg['dl_module']."_categories` b "
							. "WHERE a.`cid` = b.`cid` "
	...
	if ( !$result12 ) { //Bad SQL call
		...
		while ( $row = $pnt_db->sql_fetchrow( $result12 ) ) {
...
if ( $_POST['msnl_weblinks'] > 0 && $msnl_sTemplateNm != "notemplate" ) {
	...
	$sql				= "SELECT `lid`, a.`cid` as cid, a.`title` as ltitle, b.`title` as ctitle, `hits` FROM `"
							. $pnt_prefix."_links_links` a, `"
							. $pnt_prefix."_links_categories` b "
							. "WHERE a.`cid` = b.`cid` "
	...
	if ( !$result13 ) { //Bad SQL call
		...
		while ( $row = $pnt_db->sql_fetchrow( $result13 ) ) {
...
if ($_POST['msnl_forums'] > 0 && $msnl_sTemplateNm != "notemplate") {
	...
	$sql				= "SELECT "
							...
							."FROM `"
								.$pnt_prefix."_bbtopics` t, `"
								.$pnt_prefix."_bbforums` f, `"
								.$pnt_prefix."_bbposts` p, `"
								.$pnt_prefix."_users` ut, `"
								.$pnt_prefix."_users` up "
							."WHERE "
	...
	if ( !$result14 ) { //Bad SQL call
		...
		while ( $row = $pnt_db->sql_fetchrow( $result14 ) ) {
...
if ( $_POST['msnl_reviews'] > 0 && $msnl_sTemplateNm != "notemplate" ) {
	...
	if ( !$result15 ) { //Bad SQL call
		...
		while ( $row = $pnt_db->sql_fetchrow( $result15 ) ) {
...
if ( $_POST['msnl_banner'] != "" && $msnl_sTemplateNm != "notemplate" ) {
	...
	$sql	= "SELECT `imageurl`, `clickurl`, `alttext` FROM `"
				. $pnt_prefix."_banner` WHERE `bid` = '". $_POST['msnl_banner'] ."'";
	...
	if ( !$result16 ) { //Bad SQL call
		...
		$row = $pnt_db->sql_fetchrow( $result16 );
