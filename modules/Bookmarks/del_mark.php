...
{
	Header("Location: modules.php?name=".$titanium_module_name."&file=marks&category=".$catid."&catname=".$catname);
}

//If yes was pressed
if (isset($action)  && $action==_YES && isset($catid) && $catid!="")
{
	$delmarksquery = "delete from ".$titanium_prefix."_bookmarks where id=$markid AND user_id=$titanium_userid";	
	$titanium_db->sql_query ($delmarksquery,$titanium_db);
	$updatecatquery = "update ".$titanium_prefix."_bookmarks_cat set mod_date=now() where category_id=$catid";
